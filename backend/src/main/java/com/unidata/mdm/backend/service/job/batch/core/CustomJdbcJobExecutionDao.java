    ...
    private void insertParameter(Long executionId, ParameterType type, String key,
        ...
        try (Connection con = daoHelper.getDataSource().getConnection()) {
            ...
            if (type == ParameterType.STRING) {
                ...
                if (isArray) {
                    Array arrayArg = con.createArrayOf("timestamp", convertDate((java.util.Date[]) value));
...
//                    ExceptionId.EX_JOB_PARAMETER_PREPARE_UPSERT_ERROR, job.getId());
    ...
    private java.sql.Timestamp[] convertDate(java.util.Date[] dates) {
        java.sql.Timestamp[] result = new Timestamp[dates.length];
    ...
    private java.util.Date[] convertTimestamp(Timestamp[] timestamps) {
        java.util.Date[] result = new java.util.Date[timestamps.length];
