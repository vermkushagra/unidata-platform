 ...
 */
@Component
public class PlatformConfigurationImpl implements PlatformConfigurationExt, ConfigurationUpdatesConsumer {
    /**
     * The logger.
     */
     ...
     */
    @Override
    public ConfigurationServiceExt getConfigurationService() {
    ...
    public void subscribe(Flux<Map<String, Optional<? extends Serializable>>> updates) {
        final String simonPropertyKey = UnidataConfigurationProperty.UNIDATA_SIMON_ENABLED.getKey();
        updates
                .filter(values ->
                        values.containsKey(simonPropertyKey) && values.get(simonPropertyKey).isPresent()
                )
                .map(values -> (Boolean) values.get(simonPropertyKey).get())
                .subscribe(MeasurementPoint::setEnabled);
    }
}
