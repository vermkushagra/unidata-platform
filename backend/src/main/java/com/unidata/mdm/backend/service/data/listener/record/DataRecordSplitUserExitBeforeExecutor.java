...
public class DataRecordSplitUserExitBeforeExecutor implements DataRecordBeforeExecutor<SplitContext> {
    ...
    @Autowired
    public DataRecordSplitUserExitBeforeExecutor(ConfigurationServiceExt configurationService) {
    ...
    public boolean execute(SplitContext splitContext) {
        ...
        if (split != null) {
            Collection<BeforeSplitListener> listeners = configurationService.getListeners(
                    splitContext.getEntityName(),
                    split.getBeforeSplitInstances());

            if(CollectionUtils.isEmpty(listeners)){
                return true;
            }
            for(BeforeSplitListener listener : listeners){
