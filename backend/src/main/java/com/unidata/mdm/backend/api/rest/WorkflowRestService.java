...
import com.unidata.mdm.backend.api.rest.converter.WorkflowProcessConverter;
import com.unidata.mdm.backend.api.rest.dto.wf.WorkflowProcessRequestRO;
...
public class WorkflowRestService extends AbstractRestService {
    ...
    public Response tasks(@ApiParam(value = "Параметры") WorkflowTaskRequestRO request) {
        ...
        try {
            ...
            final GetTasksRequestContext ctx = new GetTasksRequestContextBuilder()
                    .taskId(request.getTaskId())
            ...
            return ok(
                    new RestResponse<>(
                            WorkflowStateConverter.to(workflowService.state(ctx), workflowService.currentUserCandidateGroups())
                    )
            );
        } finally {
            MeasurementPoint.stop();
    ...
    @ApiOperation(
            value = "Выводит процессы по заданным параметрам",
            notes = "Выводит процессы по заданным параметрам",
            response = WorkflowProcessStateRestResponse.class)
    @ApiResponses({
            @ApiResponse(code = 200, message = "Request processed"),
            @ApiResponse(code = 500, message = "Error occurred", response = ErrorResponse.class)
    })
    public Response processes(@ApiParam(value = "Параметры") WorkflowProcessRequestRO request) {
        ...
        try {
            ...
            GetProcessRequestContext ctx = GetProcessRequestContext
                    .builder()
                    .historical(true)
                    .processStart(processStart)
                    .initiator(request.getInitiator())
                    .involved(request.getInvolved())
                    .skipVariables(request.isSkipVariables())
                    .variables(request.getVariables())
                    .status(request.getStatus() == null
                            ? GetProcessRequestContext.Status.ALL
                            : GetProcessRequestContext.Status.valueOf(request.getStatus().name()))
                    .count(request.getCount())
                    .page(request.getPage() > 0 ? request.getPage() - 1 : request.getPage())
                    .build();

            return Response.ok(new RestResponse<>(WorkflowProcessConverter.to(workflowService.processes(ctx)))).build();
        } finally {
            MeasurementPoint.stop();
        }
    }
    ...
    public Response  assignables() {
        ...
        List<EntityInfoDefinition> result = new ArrayList<>(AbstractEntityDefToEntityListElementConverter.to(lookups));
    ...
    @ApiOperation(
            value = "Стартовать задачу",
            notes = "Стартовать задачу",
            response = RestResponse.class)
    @ApiResponses({
            @ApiResponse(code = 200, message = "Request processed"),
            @ApiResponse(code = 500, message = "Error occurred", response = ErrorResponse.class)
    ...
    //DO NOT REMOVE! This crappy workaround required for Swagger to generate API docs
    private static class WorkflowProcessStateRestResponse extends RestResponse<WorkflowProcessStateRO> {
        @Override
        public WorkflowProcessStateRO getContent() {
            return null;
        }
    }
}
