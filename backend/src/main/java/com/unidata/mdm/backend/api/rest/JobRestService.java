...
public class JobRestService extends AbstractRestService {
    ...
    @PreAuthorize("T(com.unidata.mdm.backend.service.security.utils.SecurityUtils).isAdminUser()"
            ...
            + " T(com.unidata.mdm.backend.service.security.utils.SecurityUtils).getRightsForResource('"
            + ADMIN_SYSTEM_MANAGEMENT + "').isRead()")
    @ApiOperation(value = "Экспорт операций", response = Boolean.class)
    @Path(value = "/export-jobs")
    @ApiResponses({
            @ApiResponse(code = 200, message = "Request processed"),
            @ApiResponse(code = 500, message = "Error occurred", response = ErrorResponse.class)
    })
    public Response exportJobs(List<Long> jobsIds) throws Exception {
        jobServiceExt.exportJobs(SecurityUtils.getCurrentUserName(), jobsIds);
    ...
    @PreAuthorize("T(com.unidata.mdm.backend.service.security.utils.SecurityUtils).isAdminUser()"
            ...
            + " T(com.unidata.mdm.backend.service.security.utils.SecurityUtils).getRightsForResource('"
            + ADMIN_SYSTEM_MANAGEMENT + "').isRead()")
    @ApiOperation(value = "Импорт операций", response = Boolean.class)
    ...
    public Response importJobs(
            @ApiParam(value = "Импортируемый фаил") @Multipart(value = DATA_PARAM_FILE) final Attachment fileAttachment
    ) throws Exception {
        ...
        jobServiceExt.importJobs(SecurityUtils.getCurrentUserName(), path);
    ...
    private void fillJobParametersMeta(List<JobRO> jobs) {
        ...
        for (JobRO job : jobs) {
            ...
            JobTemplateParameters parameters = jobServiceExt.findJobTemplateParameters(job.getJobNameReference());
