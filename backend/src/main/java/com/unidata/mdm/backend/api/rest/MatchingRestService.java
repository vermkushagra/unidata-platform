...
public class MatchingRestService extends AbstractRestService {
    ...
    @GET
    @Path("/group")
    @ApiOperation(value = "Получить список групп сопоставления для реестра", notes = "", response = List.class)
    @ApiResponses({
            @ApiResponse(code = 200, message = "Request processed"),
            @ApiResponse(code = 500, message = "Error occurred", response = ErrorResponse.class)
    })
    public Response getGroupsByEntityName(@ApiParam(value = "Имя реестра/справочника") @QueryParam(ENTITY_NAME) String entityName) {
        boolean all = entityName == null || entityName.isEmpty();
        Collection<MatchingGroup> matchingGroups = all ? matchingGroupsService.getAllGroups() : matchingGroupsService.getMatchingGroupsByEntityName(entityName);
        return ok(new RestResponse<>(matchingGroups.stream().map(group -> conversionService.convert(group, MatchingGroupRO.class)).collect(toList())));
    }

    @GET
    @Path("/group/{" + GROUP_ID + "}")
    @ApiOperation(value = "Обновить состав группы сопоставления", notes = "", response = RestResponse.class)
    @ApiResponses({
            @ApiResponse(code = 200, message = "Request processed"),
            @ApiResponse(code = 500, message = "Error occurred", response = ErrorResponse.class)
    })
    public Response getGroupById(@PathParam(GROUP_ID) Integer id) {
        MatchingGroup group = matchingGroupsService.getMatchingGroupsById(id);
        return ok(new RestResponse<>(conversionService.convert(group, MatchingGroupRO.class)));
    }
    ...
    @ApiOperation(value = "Создать группу настроек сопоставления", notes = "", response = RestResponse.class)
    @ApiResponses({
            @ApiResponse(code = 200, message = "Request processed"),
            @ApiResponse(code = 500, message = "Error occurred", response = ErrorResponse.class)
    })
    public Response addGroup(@Nonnull MatchingGroupRO matchingRulesGroup) {
        MatchingGroup group = conversionService.convert(matchingRulesGroup, MatchingGroup.class);
        group.setId(null);
        group = matchingGroupsService.saveMatchingGroup(group);
        return ok(new RestResponse<>(conversionService.convert(group, MatchingGroupRO.class)));
    }
    ...
    public Response updateGroup(@PathParam(GROUP_ID) Integer id, @Nonnull MatchingGroupRO matchingRulesGroup) {
        MatchingGroup group = conversionService.convert(matchingRulesGroup, MatchingGroup.class);
        group = matchingGroupsService.updateMatchingGroup(group);
        return ok(new RestResponse<>(conversionService.convert(group, MatchingGroupRO.class)));
    }
    ...
    @ApiOperation(value = "Удалить группу сопоставления", notes = "", response = RestResponse.class)
    @ApiResponses({
            @ApiResponse(code = 200, message = "Request processed"),
            @ApiResponse(code = 500, message = "Error occurred", response = ErrorResponse.class)
    })
    public Response deleteGroup(@PathParam(GROUP_ID) Integer id) {
        matchingGroupsService.removeMatchingGroup(id);
    ...
    public Response exportToXml(@ApiParam(value = "Имя реестра/справочника") @QueryParam(ENTITY_NAME) String entityName) throws Exception {
        ...
        matchingUserSettings.setMatchingGroups(all ? matchingGroupsService.getAllGroups() : matchingGroupsService.getMatchingGroupsByEntityName(entityName));
