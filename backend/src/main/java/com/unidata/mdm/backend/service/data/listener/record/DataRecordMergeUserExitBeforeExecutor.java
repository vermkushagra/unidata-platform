...
public class DataRecordMergeUserExitBeforeExecutor implements DataRecordBeforeExecutor<MergeRequestContext> {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(DataRecordMergeUserExitBeforeExecutor.class);
    /**
    ...
    public boolean execute(MergeRequestContext ctx) {
        ...
        Collection<MergeListener> listeners = configurationService.getListeners(
                ...
                upsert.getBeforeMergeInstances());
        ...
        if (configurationService.useDeprecateUserExits()) {
            for (MergeListener listener : listeners) {
                result &= listener.beforeMerge(master, duplicates, ctx);
            }
        } else {
            ExitResult exitResult = null;
            for (MergeListener listener : listeners) {
                exitResult = listener.beforeMergeWithResult(master, duplicates, ctx);
                if (exitResult == null) {
                    continue;
                }
                ...
                if (ExitResult.Status.WARNING.equals(exitResult.getStatus())) {
                    ...
                    errorInfo.setUserMessage(MessageUtils.getMessage(
                            ExceptionId.EX_MERGE_BEFORE_USER_EXIT_ERROR.getCode(),
                ...
                if (ExitResult.Status.ERROR.equals(exitResult.getStatus())) {
                    throw new DataProcessingException("Error occurred during run before merge records user exit",
                            ExceptionId.EX_MERGE_BEFORE_USER_EXIT_ERROR,
                            exitResult.getWarningMessage());
                }
            }
        }
