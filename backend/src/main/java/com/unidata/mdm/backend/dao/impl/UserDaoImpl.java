...
import com.unidata.mdm.backend.dao.rm.PasswordWithUserRowMapper;
...
import com.unidata.mdm.backend.service.security.po.BaseUserPO;
...
public class UserDaoImpl extends AbstractDaoImpl implements UserDao {
    ...
    private final String selectUsersWithProperties;

    private final String selectUsersEndpoints;

    private final String selectUsersRoles;

    private final String deleteUsersPasswords;

    private final String deleteUsersPropertiesValues;

    private final String deleteUsers;

    private final String insertUsers;

    private final String insertUsersRoles;

    private final String insertPropertiesValues;

    private final String insertUsersApis;

    private final String fetchAllUsersPasswords;

    private final String insertUsersPasswords;
     ...
     */
    @Autowired
    public UserDaoImpl(@Qualifier("unidataDataSource") DataSource dataSource, @Qualifier("security-sql") Properties sql) {
        super(dataSource);
        ...
        selectUsersWithProperties = sql.getProperty("SELECT_USERS_WITH_PROPERTIES");
        selectUsersEndpoints = sql.getProperty("SELECT_USERS_ENDPOINTS");
        selectUsersRoles = sql.getProperty("SELECT_USERS_ROLES");
        deleteUsersPasswords = sql.getProperty("DELETE_USERS_PASSWORDS");
        deleteUsersPropertiesValues = sql.getProperty("DELETE_USERS_PROPERTIES_VALUES");
        deleteUsers = sql.getProperty("DELETE_USERS");
        insertUsers = sql.getProperty("INSERT_USERS");
        insertPropertiesValues = sql.getProperty("INSERT_USERS_PROPERTIES_VALUES");
        insertUsersApis = sql.getProperty("INSERT_USERS_APIS");
        insertUsersRoles = sql.getProperty("INSERT_USERS_ROLES");
        fetchAllUsersPasswords = sql.getProperty("FETCH_ALL_USERS_PASSWORDS");
        insertUsersPasswords = sql.getProperty("INSERT_USERS_PASSWORDS");
        ...
        final Map[] links = users.stream()
                ...
                .flatMap(u ->
                        u.getApis().stream().map(a ->
                                Maps.of("login", u.getLogin(), "name", a.getName())
                        )
                )
                .toArray(Map[]::new);
