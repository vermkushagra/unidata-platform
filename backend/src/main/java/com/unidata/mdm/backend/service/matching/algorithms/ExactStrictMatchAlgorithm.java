...
public class ExactStrictMatchAlgorithm extends AbstractStrictAlgorithm {
    ...
    public FormFieldsGroup getFormFieldGroup(Map<Integer, Pair<AttributeInfoHolder, SimpleAttribute<?>>> attributeMap) {
        ...
        Pair<AttributeInfoHolder, SimpleAttribute<?>> info = attributeMap.get(ALGORITHM_ID);
        if (Objects.isNull(info.getValue()) || info.getValue().isEmpty()) {
            return createAndGroup(FormField.empty(info.getKey().getPath()));
        }

        return createAndGroup(strict(info.getValue()));
    }
