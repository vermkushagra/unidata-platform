...
public abstract class AbstractDataRecordAttributesProcessingExecutor {
    ...
    protected boolean processRecors(@Nonnull List<EtalonRecord> etalons, @Nonnull  List<OriginRecord> origins){
     ...
     */
    private void process(DataRecord record, Map<String, AttributeInfoHolder> linksEnumsOrCodeAttributes) {
        ...
        processLookupLinkDisplayValues(lookupLinkValues);
     ...
     */
    private void processLookupLinkDisplayValues(Map<AttributeInfoHolder, Set<Attribute>> values) {
        ...
        try {
            ...
            List<Pair<String, List<String>>> forRemoveCodeAttributes = new ArrayList<>();
            for (Entry<AttributeInfoHolder, Set<Attribute>> entry : values.entrySet()) {
                ...
                forRemoveCodeAttributes.add(new ImmutablePair<>(lookupEntity.getCodeAttribute().getName(), mdAttrs));
                ...
                for (Attribute value : entry.getValue()) {
                    ...
                    if (value.getAttributeType() == AttributeType.ARRAY) {
                        searchValues = ((ArrayAttribute<?>) value).isEmpty()
                    ...
                    SearchRequestContext searchCtx = forEtalonData(lookupName)
                            ...
                            .searchFields(singletonList(lookupEntity.getCodeAttribute().getName()))
            ...
            for(Pair<String, List<String>> forRemove : forRemoveCodeAttributes){
                forRemove.getRight().remove(forRemove.getRight().lastIndexOf(forRemove.getLeft()));
            }
            ...
            for (Entry<SearchRequestContext, SearchResultDTO> entry : response.entrySet()) {
                ...
                if (attrValue.getAttributeType() == AttributeType.ARRAY) {
                    for (ArrayValue<?> a : ((ArrayAttribute<?>) attrValue).getValue()) {
                        SearchResultHitDTO hit = findHit(result, a.getValue(), ctx.getSearchFields().get(0));
                        String displayValue = extractSearchResult(hit, ctx.getReturnFields(), attrHolder);
                        a.setDisplayValue(displayValue);
                    }
                } else {
                    SimpleAttribute<?> simpleAttribute = (SimpleAttribute<?>) attrValue;
                    SearchResultHitDTO hit = findHit(result, simpleAttribute.getValue(), ctx.getSearchFields().get(0));
                    String displayValue = extractSearchResult(hit, ctx.getReturnFields(), attrHolder);
                    simpleAttribute.setDisplayValue(displayValue);
                    ((CodeLinkValue) attrValue).setLinkEtalonId(displayValue != null ? hit.getId() : null);
                }
            }
        } finally {
            MeasurementPoint.stop();
     ...
     */
    private String extractSearchResult(SearchResultHitDTO hit, List<String> fieldNames, AttributeInfoHolder attrHolder) {
     ...
     */
    private SearchResultHitDTO findHit(SearchResultDTO result, Object value, String codeName) {
    ...
    private void filterAttributesNotFoundInModelRecursive(Map<String, AttributeInfoHolder> map, DataRecord record, String path) {
        ...
        while (it.hasNext()) {
            ...
            if (attr.getAttributeType() == AttributeType.COMPLEX) {
                ...
                for (DataRecord nested : cattr.getRecords()) {
