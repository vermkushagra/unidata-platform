...
package com.unidata.mdm.backend.service.data.upath.impl;
...
import com.unidata.mdm.backend.common.upath.UPathConstants;
import com.unidata.mdm.backend.common.upath.UPathElement;
import com.unidata.mdm.backend.common.upath.UPathElementType;
...
public class UPathServiceImpl implements UPathService {
     ...
     */
    @Override
    public UPath upathCreate(String entity, String path) {
        return upathCreate(entity, path, metaModelService.getAttributesInfoMap(entity));
    }
    /**
     * {@inheritDoc}
     */
    @Override
    public UPath upathCreate(String entity, String path, Map<String, AttributeInfoHolder> info) {
        ...
        if (StringUtils.isBlank(entity) || StringUtils.isBlank(path)) {
            ...
            throw new UPathException(message, ExceptionId.EX_UPATH_INVALID_INPUT_ENTITY_OR_PATH_BLANK, entity, path);
        }

        if (MapUtils.isEmpty(info)) {
            final String message = "Entity not found by name [{}].";
            LOGGER.warn(message, entity);
            throw new UPathException(message, ExceptionId.EX_UPATH_ENTITY_NOT_FOUND_BY_NAME, entity);
        }
        ...
        if (pathTokens.length == 0) {
            ...
            throw new UPathException(message, ExceptionId.EX_UPATH_INVALID_INPUT_SPLIT_TO_ZERO_ELEMENTS, path);
        }
        ...
        for (int i = 0; i < pathTokens.length; i++) {
            ...
            AttributeInfoHolder check = checkPathComponent(upath, info, token);
     ...
     */
    @Override
    public UPathResult upathResult(String entity, String path, DataRecord record) {
        UPath upath = upathCreate(entity, path);
                ...
                for (Iterator<DataRecord> li = holder.iterator(); li.hasNext(); ) {
                    ...
                    if (Objects.nonNull(attr)) {
                        ...
                        if (!isTerminating && !isComplex) {
                            ...
                            throw new UPathException(message,
                                    ExceptionId.EX_UPATH_NOT_A_COMPLEX_ATTRIBUTE_FOR_INTERMEDIATE_PATH_ELEMENT,
                                    element.getElement());
                        }
                ...
                for (Iterator<DataRecord> li = holder.iterator(); li.hasNext(); ) {
                    ...
                    if (Objects.nonNull(attr)) {
                        ...
                        if (!isComplex) {
                            final String message = "Attribute selected for an intermediate path element [{}] is not a complex attribute.";
                            LOGGER.warn(message, element.getElement());
                            throw new UPathException(message, ExceptionId.EX_UPATH_INVALID_SET_NOT_A_COMPLEX_FOR_INTERMEDIATE, element.getElement());
                        }
    ...
    private void checkGeneralSetAbility(UPathElement last, Attribute target) {
        ...
        if (last.getType() != UPathElementType.COLLECTING) {
            ...
            throw new UPathException(message, ExceptionId.EX_UPATH_INVALID_SET_WRONG_END_ELEMENT, last.getElement());
        }
        ...
        if (!isSetOperationTypeConform(target, last.getInfo())) {
            ...
            LOGGER.warn(message, last.getElement());
            throw new UPathException(message, ExceptionId.EX_UPATH_INVALID_SET_WRONG_TARGET_ATTRIBUTE_TYPE, last.getElement());
        }
        ...
        if (!StringUtils.equals(last.getElement(), target.getName())) {
            ...
            throw new UPathException(message, ExceptionId.EX_UPATH_INVALID_SET_WRONG_ATTRIBUTE_NAME, target.getName(), last.getElement());
        }
    }
    /**
     ...
     */
    private boolean checkRootRecord(UPath upath, String element, Map<String, AttributeInfoHolder> info) {
        ...
        if (end == -1) {
            ...
            throw new UPathException(message, ExceptionId.EX_UPATH_INVALID_ROOT_EXPRESSION, element);
        }
     ...
     */
    private boolean checkSubscriptFilter(UPath upath, String element, Map<String, AttributeInfoHolder> info) {
        ...
        if (end == -1) {
            ...
            LOGGER.warn(message, element);
            throw new UPathException(message, ExceptionId.EX_UPATH_INVALID_SUBSCRIPT_EXPRESSION, element);
        }
        ...
        AttributeInfoHolder checked = checkPathComponent(upath, info, subpath);
     ...
     */
    private boolean checkExpressionFilter(UPath upath, String element, Map<String, AttributeInfoHolder> info) {
        ...
        if (StringUtils.isNotBlank(name)) {
            AttributeInfoHolder check = checkPathComponent(upath, info, name);
     ...
     * @return element
     */
    private UPathElement createExpressionFilter(Map<String, AttributeInfoHolder> info, UPath upath, String key, String value) {

        String path = joinNonBlank(".", upath.toPath(), key);
        AttributeInfoHolder aih = info.get(path);
        if (Objects.isNull(aih)) {
            final String message = "Invalid input. Filtering expression denotes attribute not found in model [{}].";
            LOGGER.warn(message, path);
            throw new UPathException(message, ExceptionId.EX_UPATH_INVALID_FILTERING_EXPRESSION_MISSING_ATTRIBUTE, path);
        }

        Predicate<DataRecord> predicate = null;
        SimpleDataType type = selectDataType(aih);
        ...
        if (Objects.isNull(predicate)) {
            ...
            throw new UPathException(message, ExceptionId.EX_UPATH_INVALID_FILTERING_EXPRESSION_ATTRIBUTE_TYPE, key);
        }
    ...
    private Predicate<DataRecord> createDateExpressionPredicate(String key, String value) {
        ...
        try {
            ...
            throw new UPathException(e, ExceptionId.EX_UPATH_INVALID_FILTERING_EXPRESSION_DATE_FORMAT, value);
        }
        ...
        return dr -> {
            ...
            if (a.getAttributeType() == AttributeType.SIMPLE) {
                ...
                if (sa.getDataType() == DataType.DATE) {
                    ...
                    return ld == null ? false : match.equals(ld);
                }
    ...
    private Predicate<DataRecord> createTimeExpressionPredicate(String key, String value) {
        ...
        try {
            ...
            LOGGER.warn(message, value);
            throw new UPathException(e, ExceptionId.EX_UPATH_INVALID_FILTERING_EXPRESSION_TIME_FORMAT, value);
        }
    ...
    private Predicate<DataRecord> createTimestampExpressionPredicate(String key, String value) {
        ...
        try {
            ...
            LOGGER.warn(message, value);
            throw new UPathException(e, ExceptionId.EX_UPATH_INVALID_FILTERING_EXPRESSION_TIMESTAMP_FORMAT, value);
        }
    ...
    private Predicate<DataRecord> createNumericExpressionPredicate(String key, String value) {
        ...
        try {
            ...
            LOGGER.warn(message, value);
            throw new UPathException(e, ExceptionId.EX_UPATH_INVALID_FILTERING_EXPRESSION_NUMBER_FORMAT, value);
        }
        ...
        return dr -> {
            ...
            if (a.getAttributeType() == AttributeType.SIMPLE) {
                ...
                if (sa.getDataType() == DataType.INTEGER) {
                    Long l = sa.castValue();
                    return l == null ? false : match.longValue() ==l.longValue();
                } else if (sa.getDataType() == DataType.NUMBER) {
                    Double d = sa.castValue();
                    return d == null ? false : match.doubleValue() == d.doubleValue();
                }
            } else if (a.getAttributeType() == AttributeType.CODE) {
                CodeAttribute<Long> ca = a.narrow();
                return ca.getValue() == null ? false : match.longValue() == ca.getValue().longValue();
            } else if (a.getAttributeType() == AttributeType.ARRAY) {
    ...
    private Predicate<DataRecord> createStringExpressionPredicate(String key, String value) {
        ...
        if (value.length() < 2
         ...
         || value.charAt(value.length() - 1) != '\'') {
            ...
            LOGGER.warn(message, value);
            throw new UPathException(message, ExceptionId.EX_UPATH_INVALID_FILTERING_EXPRESSION_STRING_FORMAT, value);
        }
    ...
    /**
     * Selects the data type for expression.
     * @param holder attribute holder
     * @return SDT
     */
    private SimpleDataType selectDataType(AttributeInfoHolder holder) {

        if (holder.isCode()) {

            CodeAttributeDef cad = holder.narrow();
            return cad.getSimpleDataType();
        } else if (holder.isSimple()) {

            if (holder.isEnumValue()) {
                return SimpleDataType.STRING;
            }

            SimpleAttributeDef sad = holder.narrow();
            if (holder.isLookupLink()) {
                return sad.getLookupEntityCodeAttributeType();
            }

            return sad.getSimpleDataType();
        } else if (holder.isArray()) {

            ArrayAttributeDef aad = holder.narrow();
            ArrayValueType type;
            if (holder.isLookupLink()) {
                type = aad.getLookupEntityCodeAttributeType();
            } else {
                type = aad.getArrayValueType();
            }
            ...
            final String message = "Invalid input. Filtering expression denotes complex attribute as filter attribute [{}]. "
                    + "Filter attribute may be either simple or code or array.";
            LOGGER.warn(message, holder.getPath());
            throw new UPathException(message, ExceptionId.EX_UPATH_INVALID_FILTERING_EXPRESSION_COMPLEX_ATTRIBUTE, holder.getPath());
        }

        return null;
    }

    private static AttributeInfoHolder checkPathComponent(UPath upath, Map<String, AttributeInfoHolder> info, String token) {

        String currentPath = joinNonBlank(".", upath.toPath(), token);
        AttributeInfoHolder check = info.get(currentPath);
        if (Objects.isNull(check)) {
            final String message = "Invalid input. Attribute not found by path [{}].";
            LOGGER.warn(message, currentPath);
            throw new UPathException(message, ExceptionId.EX_UPATH_INVALID_INPUT_ATTRIBUTE_NOT_FOUND_BY_PATH, currentPath);
        }
     ...
     */
    private boolean isSetOperationTypeConform(Attribute check, AttributeInfoHolder info) {

        if (check.getAttributeType() == AttributeType.ARRAY && info.isArray()) {

            ArrayAttributeDef aad = info.narrow();
            ArrayAttribute<?> aa = (ArrayAttribute<?>) check;

            switch (aa.getDataType()) {
            case DATE:
                return aad.getArrayValueType() == ArrayValueType.DATE;
            case INTEGER:
                return aad.getArrayValueType() == ArrayValueType.INTEGER
                    || (info.isLookupLink() && aad.getLookupEntityCodeAttributeType() == ArrayValueType.INTEGER);
            case NUMBER:
                return aad.getArrayValueType() == ArrayValueType.NUMBER;
            case STRING:
                return aad.getArrayValueType() == ArrayValueType.STRING
                    || info.isEnumValue()
                    || info.isLinkTemplate()
                    || (info.isLookupLink() && aad.getLookupEntityCodeAttributeType() == ArrayValueType.STRING);
            case TIME:
                return aad.getArrayValueType() == ArrayValueType.TIME;
            case TIMESTAMP:
                return aad.getArrayValueType() == ArrayValueType.TIMESTAMP;
            default:
                break;
            }
            ...
            CodeAttributeDef cad = info.narrow();
            CodeAttribute<?> ca = (CodeAttribute<?>) check;

            switch (ca.getDataType()) {
            case INTEGER:
                return cad.getSimpleDataType() == SimpleDataType.INTEGER;
            case STRING:
                return cad.getSimpleDataType() == SimpleDataType.STRING;
            default:
                break;
            }
            ...
            case BLOB:
                return info.isBlob();
            case BOOLEAN:
                return sad.getSimpleDataType() == SimpleDataType.BOOLEAN;
            case CLOB:
                return info.isClob();
            case DATE:
                return sad.getSimpleDataType() == SimpleDataType.DATE;
            case INTEGER:
                return sad.getSimpleDataType() == SimpleDataType.INTEGER
                    || (info.isLookupLink() && sad.getLookupEntityCodeAttributeType() == SimpleDataType.INTEGER);
            case MEASURED:
                return sad.getSimpleDataType() == SimpleDataType.MEASURED;
            case NUMBER:
                return sad.getSimpleDataType() == SimpleDataType.NUMBER;
            case STRING:
                return sad.getSimpleDataType() == SimpleDataType.STRING
                    || info.isEnumValue()
                    || info.isLinkTemplate()
                    || (info.isLookupLink() && sad.getLookupEntityCodeAttributeType() == SimpleDataType.STRING);
            case TIME:
                return sad.getSimpleDataType() == SimpleDataType.TIME;
            case TIMESTAMP:
                return sad.getSimpleDataType() == SimpleDataType.TIMESTAMP;
            default:
                break;
            }
