...
public class ModifyItemWriter extends AbstractUnidataWriter<Pair<ImportRecordSet, ImportRelationSet>> {
    ...
    private void processClassifiers(
            final List<UpsertClassifiersDataRequestContext> upsertClassifiers,
            final List<DeleteClassifiersDataRequestContext> deleteClassifiers, ModifyItemJobStepExecutionState state
    ) {
        try {
            List<DeleteClassifiersDTO> deleteResult = classifiersServiceComponent.batchDeleteClassifiers(deleteClassifiers, false);
            ...
            long deletedClassifiers = deleteResult.stream()
                    .mapToLong(value -> value.getClassifiers()
                            .values()
                            .stream()
                            .mapToLong(value1 -> (long) value1.size())
                            .sum())
                    .sum();
        ...
        try {
            List<UpsertClassifiersDTO> upsertResult = classifiersServiceComponent.batchUpsertClassifiers(upsertClassifiers, false);
    ...
    private void processRecords(List<UpsertRequestContext> upsertRecords, ModifyItemJobStepExecutionState state) {
        try {
            List<UpsertRecordDTO> records = recordsServiceComponent.batchUpsertRecords(upsertRecords, false);
    ...
    private void processRelations(List<UpsertRelationsRequestContext> upsertRelations,
        ...
        try {
            List<UpsertRelationsDTO> records = relationsServiceComponent.batchUpsertRelations(upsertRelations, false);
        ...
        try {
            List<DeleteRelationsDTO> records = relationsServiceComponent.batchDeleteRelations(deleteRelations, false);
