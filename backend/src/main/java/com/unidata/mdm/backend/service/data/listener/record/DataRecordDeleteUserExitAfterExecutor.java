 ...
 */
public class DataRecordDeleteUserExitAfterExecutor
    implements DataRecordAfterExecutor<DeleteRequestContext>, AbstractDataRecordDeleteCommonExecutor<DeleteRequestContext> {

    private static final Logger LOGGER = LoggerFactory.getLogger(DataRecordDeleteUserExitAfterExecutor.class);
    ...
    public boolean execute(DeleteRequestContext ctx) {
        ...
        try {
            ...
            Collection<DeleteListener> listeners = configurationService.getListeners(
                    ...
                    delete.getAfterEtalonDeactivationInstances());
            ...
            if (configurationService.useDeprecateUserExits()) {
                listeners.forEach(listener -> listener.afterEtalonDeactivation(etalon, ctx));
            } else {

                ExitResult exitResult = null;
                for (DeleteListener listener : listeners) {
                    exitResult = listener.afterEtalonDeactivationWithResult(etalon, ctx);
                    if (exitResult == null) {
                        continue;
                    }
                    ...
                    if (ExitResult.Status.WARNING.equals(exitResult.getStatus())) {
                        ...
                        errorInfo.setUserMessage(MessageUtils.getMessage(
                                ExceptionId.EX_DATA_DELETE_RECORD_AFTER_USER_EXIT_ERROR.getCode(),
                    ...
                    if (ExitResult.Status.ERROR.equals(exitResult.getStatus())) {
                        throw new DataProcessingException("Error occurred during run after delete record user exit",
                                ExceptionId.EX_DATA_DELETE_RECORD_AFTER_USER_EXIT_ERROR,
                                exitResult.getWarningMessage());
                    }
                }
            }
