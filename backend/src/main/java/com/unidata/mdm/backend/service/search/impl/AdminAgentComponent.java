...
public class AdminAgentComponent extends BaseAgentComponent {
    ...
    public static final String STRING_VALUE_FIELD = ClassifierDataHeaderField.FIELD_CLS_ATTR_VALUE_PREFIX.getField() + "string";
    public static final String DOUBLE_VALUE_FIELD = ClassifierDataHeaderField.FIELD_CLS_ATTR_VALUE_PREFIX.getField() + "double";
    public static final String BOOLEAN_VALUE_FIELD = ClassifierDataHeaderField.FIELD_CLS_ATTR_VALUE_PREFIX.getField() + "boolean";
    public static final String DATE_VALUE_FIELD = ClassifierDataHeaderField.FIELD_CLS_ATTR_VALUE_PREFIX.getField() + "date";
    public static final String TIME_VALUE_FIELD = ClassifierDataHeaderField.FIELD_CLS_ATTR_VALUE_PREFIX.getField() + "time";
    public static final String INTEGER_VALUE_FIELD = ClassifierDataHeaderField.FIELD_CLS_ATTR_VALUE_PREFIX.getField() + "long";
    /**
    ...
    public boolean index(final List<IndexRequestContext> ctxts) {
        ...
        Collection<InfoSection> unspecifiedMatchingDeletes = new ArrayList<>();
        for (IndexRequestContext ctx : ctxts) {
            ...
            unspecifiedMatchingDeletes.addAll(ctx.getMatchingToQueryDelete());
        ...
        specifyDeletes(builder, unspecifiedDeletes, unspecifiedMatchingDeletes);
    ...
    public boolean index(final IndexRequestContext ctx) {
        ...
        Collection<InfoSection> unspecifiedDeletes = new ArrayList<>();
        Collection<InfoSection> unspecifiedMatchingDeletes = new ArrayList<>(ctx.getMatchingToQueryDelete());
     ...
     * @return true, if successful, false otherwise
     */
    public boolean mark(final SearchRequestContext ctx, Map<? extends SearchField, Object> fields, Boolean refreshImmediate) {
        // 1. Compose the name of the type
        final String indexName = constructIndexName(ctx);
     ...
     * @return true, if successful, false otherwise
     */
    public boolean mark(final ComplexSearchRequestContext ctx, Map<? extends SearchField, Object> fields, Boolean refreshImmediate) {
        // 1. Compose the name of the type
    ...
    private boolean updateInternal(@Nonnull QueryBuilder query, @Nonnull XContentBuilder update,
                                   @Nullable com.unidata.mdm.backend.common.search.types.SearchType searchType, Boolean refreshImmediate, final String... indexNames) {
    ...
    private XContentBuilder buildObject(
            ...
            List<DataQualityError> dqErrors) {

        try (XContentBuilder builder = XContentFactory.jsonBuilder()) {
            builder.startObject();
            buildHeader(builder, header);
            buildRecord(builder, object, null);
            buildDQErrors(builder, dqErrors);
            builder.endObject();
            return builder;
    ...
    private XContentBuilder buildRelation(EtalonRelation etalonRelation, Boolean withData) {
        ...
        try (XContentBuilder builder = XContentFactory.jsonBuilder()) {
            ...
            builder.field(RelationHeaderField.FIELD_PERIOD_ID.getField(), PeriodIdUtils.periodIdFromDate(infoSection.getValidTo()));
    ...
    private void buildHeader(XContentBuilder builder, final Map<? extends SearchField, Object> header) throws IOException {
        ...
        for (Entry<? extends SearchField, Object> entry : header.entrySet()) {
            if (entry.getKey() == RecordHeaderField.FIELD_FROM) {
    ...
    private XContentBuilder buildClassifier(@Nonnull EtalonClassifier classifier) {
        ...
        try (XContentBuilder builder = XContentFactory.jsonBuilder()) {
            ...
            if(CollectionUtils.isNotEmpty(classifier.getAllAttributes())){
                builder.startArray(ClassifierDataHeaderField.FIELD_CLS_NESTED_ATTRS.getField());
                for (Attribute attr : classifier.getAllAttributes()) {
                    if (attr == null) {
                        continue;
                    }
                    String attrPath = ModelUtils.getAttributePath(null, attr.getName());
                    builder.startObject();
                    builder.field(ClassifierDataHeaderField.FIELD_CLS_ATTR_NAME.getField() , attrPath);
                    buildClassifierAttribute(builder, attr, attrPath);
                    builder.endObject();
                }
                builder.endArray();
            }
    ...
    private void buildRecord(XContentBuilder builder, DataRecord record, @Nullable String recordPath) throws IOException {
        for (Attribute attr : record.getAllAttributes()) {
            ...
            buildAttribute(builder, attr, attrPath);
        }
    }

    private void buildClassifierAttribute(XContentBuilder builder, Attribute attr, @Nonnull String attrPath) throws IOException {
        ...
        switch (attr.getAttributeType()) {
            case SIMPLE:
                ...
                switch (sAttr.getDataType()) {
                    ...
                    case LINK:
                    case ENUM:
                        builder.field(STRING_VALUE_FIELD, sAttr.getValue() == null || sAttr.getValue().toString().isEmpty() ?
                                null :
                                (String) sAttr.getValue());
                        break;
                    case NUMBER:
                    case MEASURED:
                        builder.field(DOUBLE_VALUE_FIELD, sAttr.getValue() == null ? null : (Double) sAttr.getValue());
                        break;
                    case BOOLEAN:
                        builder.field(BOOLEAN_VALUE_FIELD, sAttr.getValue() == null ? null : (Boolean) sAttr.getValue());
                        break;
                    case DATE:
                        builder.field(DATE_VALUE_FIELD, sAttr.getValue() == null ?
                                null :
                                DateTimeFormatter.ISO_LOCAL_DATE.format((LocalDate) sAttr.getValue()));
                        break;
                    case TIME:
                        builder.field(TIME_VALUE_FIELD, sAttr.getValue() == null ?
                                null :
                    ...
                    case TIMESTAMP:
                        ...
                        break;
                    case INTEGER:
                        builder.field(INTEGER_VALUE_FIELD, sAttr.getValue() == null ? null : (Long) sAttr.getValue());
                        break;
                    case BLOB:
                        builder.field(STRING_VALUE_FIELD, sAttr.getValue() == null ? null : ((BinaryLargeValue) sAttr.getValue()).getFileName());
                        break;
                    case CLOB:
                        builder.field(STRING_VALUE_FIELD, sAttr.getValue() == null ? null : ((CharacterLargeValue) sAttr.getValue()).getFileName());
                        break;
                }
                break;
            case CODE:
            ...
            case COMPLEX:
                ...
                for (DataRecord nested : complexAttribute) {
     ...
     * @throws IOException
     */
    private void buildAttribute(XContentBuilder builder, Attribute attr, @Nonnull String attrPath) throws IOException {
        final String name = attr.getName();
        switch (attr.getAttributeType()) {
            case SIMPLE:
                SimpleAttribute<?> sAttr = (SimpleAttribute<?>) attr;
     ...
     * Create DQ errors.
     *
     * @param error the dq error
     * @param infoSection the info section of the record, being indexed
     * @throws IOException
     */
    /*
    private XContentBuilder buildDqError(final DataQualityError error, EtalonRecordInfoSection infoSection) {
        try (final XContentBuilder builder = XContentFactory.jsonBuilder()) {

            long point = System.currentTimeMillis();
            builder.startObject()
                    // .field(DqHeaderField.ETALON_ID.getDirectField(), Objects.nonNull(error.getEtalonId()) ? error.getEtalonId() : infoSection.getEtalonKey().getId())
                    .field(DqHeaderField.ERROR_ID.getDirectField(), error.getId())
                    .field(DqHeaderField.CREATE_DATE.getDirectField(), Objects.nonNull(error.getCreateDate()) ? SearchUtils.parseForIndex(error.getCreateDate()) : SearchUtils.parseForIndex(new Date(point)))
                    .field(DqHeaderField.UPDATE_DATE.getDirectField(), Objects.nonNull(error.getUpdateDate()) ? SearchUtils.parseForIndex(error.getUpdateDate()) : SearchUtils.parseForIndex(new Date(point)))
                    ...
                    .field(DqHeaderField.PATHS.getDirectField(), error.getPaths())
                    .endObject();
            return builder;
        }
        catch (IOException e) {
    ...
    */
     ...
     * @throws IOException
     */
    private void buildDQErrors(XContentBuilder builder, List<DataQualityError> errors) {
        ...
        if (!CollectionUtils.isEmpty(errors)) {
            try {
                ...
                for (DataQualityError error : errors) {
                    builder
                        .startObject()
                            ...
                            .field(DQHeaderField.SEVERITY.getDirectField(), error.getSeverity().name())
                            .field(DQHeaderField.CATEGORY.getDirectField(), error.getCategory())
                            .field(DQHeaderField.EXECUTION_MODE.getField(), error.getExecutionMode().name())
                            .field(DQHeaderField.PATHS.getDirectField(), error.getValues().stream().map(Pair::getKey).collect(Collectors.toList()))
                        .endObject();
                }
                ...
                final String message = "Failed to index DQ errors. {}.";
                LOGGER.warn(message, e);
                throw new SearchApplicationException(message, e,
                        ExceptionId.EX_SEARCH_BUILD_DQ_ERRORS_FAILED);
            }
        }
    }
    /**
    ...
    private Pair<Collection<InfoSection>, Collection<DeleteRequestBuilder>> processDelete(final IndexRequestContext ctx) {
        ...
        List<DeleteRequestBuilder> straightDelete = new ArrayList<>(
                ctx.getRecordsToDelete().size()
              + ctx.getClassifiersToDelete().size()
              + ctx.getRelationsToDelete().size());

        ctx.getDeleteIdsGroupedByEntityName().forEach((entityName, ids) -> {
            String targetIndexName =  constructIndexName(entityName, null);
            ids.forEach(id ->
                straightDelete.add(
                    client.prepareDelete(targetIndexName,
                            id.getSearchType().getName(), id.getIndexId()).setRouting(id.getRouting()))
            );
        });
        ...
        return new ImmutablePair<>(queryDelete, straightDelete);
    }
    ...
    private Collection<IndexRequestBuilder> processIndex(final IndexRequestContext ctx) {
        ...
        for (Entry<EtalonRecord, Map<? extends SearchField, Object>> e : ctx.getRecords().entrySet()) {
            ...
            XContentBuilder xcb = buildObject(etalonRecord, e.getValue(), ctx.getDqErrors().get(etalonRecord));
            final IndexRequestBuilder record = client.prepareIndex(
                    indexName,
                    EntitySearchType.ETALON_DATA.getName(),
                    PeriodIdUtils.childPeriodId(infoSection.getPeriodId(), infoSection.getEtalonKey().getId()))
                    .setSource(xcb)
                    .setParent(infoSection.getEtalonKey().getId());
            result.add(record);

            // Update for each request (think about optimistic lock for control indexing)
            if (!etalonSet) {
                IndexRequestBuilder etalon = client.prepareIndex(indexName, EntitySearchType.ETALON.getName(),
                        infoSection.getEtalonKey().getId())
                        .setSource(Collections.<String, Object>emptyMap());
                result.add(etalon);
                etalonSet = true;
            }
        }
        ...
        for (EtalonRelation relation : ctx.getRelations()) {
            ...
            IndexRequestBuilder straightRelation = client.prepareIndex(indexName, EntitySearchType.ETALON_RELATION.getName(),
                    PeriodIdUtils.childPeriodId(infoSection.getPeriodId(), fromEtalonId, infoSection.getRelationName(), toEtalonId))
            ...
            IndexRequestBuilder revertRelation = client.prepareIndex(linkedIndex, EntitySearchType.ETALON_RELATION.getName(),
                    PeriodIdUtils.childPeriodId(infoSection.getPeriodId(), toEtalonId,  infoSection.getRelationName(), fromEtalonId))
        ...
        for (EtalonClassifier classifier : ctx.getClassifiers()) {
            ...
            IndexRequestBuilder classifierData = client.prepareIndex(indexName, EntitySearchType.CLASSIFIER.getName(),
                    PeriodIdUtils.childPeriodId(recordEtalonId, infoSection.getClassifierName(), infoSection.getNodeId())) // Classifier data doesn't have periods
    ...
    private void specifyDeletes(BulkRequestBuilder bulk, Collection<InfoSection> unspecifiedDeletes, Collection<InfoSection> unspecifiedMatching) {
        Map<String, Map<EntitySearchType, List<String>>> ids = new HashMap<>();
        Map<String, InfoSection> content = new HashMap<>();

        if (CollectionUtils.isNotEmpty(unspecifiedDeletes)) {
        ...
        if (CollectionUtils.isNotEmpty(unspecifiedMatching)) {
            for (InfoSection unspecifiedDelete : unspecifiedMatching) {

                if (unspecifiedDelete.getRecordType() == RecordType.DATA_RECORD) {
                    EtalonRecordInfoSection eris = (EtalonRecordInfoSection) unspecifiedDelete;
                    ids
                            .computeIfAbsent(eris.getEntityName(), k -> new EnumMap<>(EntitySearchType.class))
                            .computeIfAbsent(EntitySearchType.MATCHING, k -> new ArrayList<>())
                            .add(eris.getEtalonKey().getId());
                    content.put(eris.getEtalonKey().getId(), unspecifiedDelete);
                }
            }
                ...
                .map(entry -> entry.getValue().entrySet().stream()
                        .map(child -> {
                            ...
                            if (child.getKey() == EntitySearchType.ETALON_DATA) {
                                etalonIdField = RecordHeaderField.FIELD_ETALON_ID.getField();
                            } else if (child.getKey() == EntitySearchType.MATCHING) {
                                etalonIdField = MatchingHeaderField.FIELD_ETALON_ID.getField();
    ...
    processUpdate(IndexRequestContext ctx) {
        ...
        for (EtalonRecordInfoSection is : ctx.getRecordsToSysUpdate()) {
            ...
            try {
                ...
                if (Objects.nonNull(is.getOperationType())) {
                    builder.field(RecordHeaderField.FIELD_OPERATION_TYPE.getField(), is.getOperationType());
                }
            ...
            if (Objects.nonNull(is.getPeriodId())) {
                ...
                ur.id(PeriodIdUtils.childPeriodId(is.getPeriodId(), is.getEtalonKey().getId()));
