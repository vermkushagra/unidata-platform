...
public class WorkflowChangeListenerImpl implements ActivitiEventListener {
    ...
    static {
        STATUS_TAGS = new ArrayList<>();
        STATUS_TAGS.add(RecordStatus.ACTIVE);
        STATUS_TAGS.add(RecordStatus.INACTIVE);
    }
    /**
     * Data record DAO.
     */
    ...
    public void completeProcess(ActivitiEvent evt) {
        ...
        evt.getEngineServices().getRuntimeService().setVariable(evt.getExecutionId(),
                WorkflowVariables.VAR_PROCESS_COMPLETED.getValue(),
                state.isComplete());
    ...
    private void approveRecord(String etalonId, String operationId) {
        ...
        try {
            ...
            // 8. Approve classifiers
            approveClassifiers(keys, operationId, hasActiveVersions);
    ...
    private void declineRecord(String etalonId, String operationId) {
        ...
        try {
            ...
            // 3. Create context
            boolean hasActiveVersions = keys != null && hasActiveRecordVersions(etalonId, keys.getEntityName());
            DeleteRequestContext ctx = new DeleteRequestContextBuilder()
                    .etalonKey(etalonId)
                    .inactivateEtalon(hasActiveVersions)
                    .cascade(true)
            ...
            // 6. Decline classifiers
            declineClassifiers(keys, operationId, hasActiveVersions);

            // 7. Cleanup drafts
            dataRecordsDao.cleanupEtalonStateDrafts(etalonId);

            // 8. Change record state (wipe or just reset)
            if (hasActiveVersions) {
                // 8.1 Update index
                Map<RecordHeaderField, Object> fields = Collections.singletonMap(RecordHeaderField.FIELD_PENDING, FALSE);
                searchService.mark(keys.getEntityName(), etalonId, fields);
    ...
    /**
     * Tells, whether this classifier etalon has active versions or not.
     * @param etalonId classifier etalon id
     * @return true, if has, false otherwise
     */
    private boolean hasActiveClassifierVersions(String etalonId) {

        // CLSF data has one single period so far
        List<Triple<String, RecordStatus, ApprovalState>> versionInfo
            = classifiersDao.loadActiveInactiveClassifierVersionsInfo(etalonId);

        for (Triple<String, RecordStatus, ApprovalState> info : versionInfo) {
            if (info.getRight() == ApprovalState.APPROVED
             && info.getMiddle() == RecordStatus.ACTIVE) {
                return true;
            }
        }
    ...
    /**
     * Approves classifier data.
     * @param keys the keys
     * @param operationId operation id
     * @param parentHasActiveVersions whether parent has active versions
     */
    private void approveClassifiers(RecordKeys keys, String operationId, boolean parentHasActiveVersions) {

        List<String> classifierNames = metaModelService.getClassifiersForEntity(keys.getEntityName());
        for (String classifierName : classifierNames) {

            List<EtalonClassifierPO> toEtalons
                    = classifiersDao.loadClassifierEtalons(keys.getEtalonKey().getId(), classifierName, STATUS_TAGS);

            if (CollectionUtils.isEmpty(toEtalons)) {
                continue;
            }

            for (EtalonClassifierPO po : toEtalons) {

                if (po.getApproval() != ApprovalState.PENDING) {
                    continue;
                }

                updateClassifierApprovalState(po.getId(), ApprovalState.APPROVED);
                EtalonClassifierDraftStatePO draft = classifiersDao.loadLastEtalonStateDraft(po.getId());
                ...
                if (Objects.nonNull(draft) && draft.getStatus() != po.getStatus()) {
                    ...
                    po.setUpdateDate(null);
                    po.setUpdatedBy(SecurityUtils.getCurrentUserName());
                    classifiersDao.upsertEtalonClassifier(po, false);
                }

                classifiersDao.cleanupEtalonStateDrafts(po.getId());

                boolean doDelete = keys.getEtalonStatus() == RecordStatus.INACTIVE && po.getStatus() != RecordStatus.INACTIVE;
                boolean doWipe = !parentHasActiveVersions || !hasActiveClassifierVersions(po.getId());
                if (doDelete || doWipe) {
                    ...
                    classifiersServiceComponent.deleteClassifier(dCtx);
                }
            }
        }
    }
     ...
     * @param parentHasActiveVersions whether parent has active versions
     */
    private void declineClassifiers(RecordKeys keys, String operationId, boolean parentHasActiveVersions) {
        ...
        for (String classifierName : classifierNames) {
            ...
            for (EtalonClassifierPO po : toEtalons) {
                ...
                updateClassifierApprovalState(po.getId(), ApprovalState.DECLINED);
                classifiersDao.cleanupEtalonStateDrafts(po.getId());
    ...
    /**
     * Approves pending versions.
     * @param classifierEtalonId the etalonn id
     * @param state the state to set
     */
    private void updateClassifierApprovalState(String classifierEtalonId, ApprovalState state) {

        // 1. Update state of relations versions
        classifiersDao.changeVersionsApprovalState(classifierEtalonId, state);

        // 2. Update relation record
        classifiersDao.changeEtalonApprovalState(classifierEtalonId, state == ApprovalState.PENDING ? state : ApprovalState.APPROVED);
    }
