...
public abstract class XLSXProcessor {
    ...
    /**
     * The Import_ID, constructed of ID and EXTERNAL_ID or random generated
     */
    protected static final String XLSX_IMPORT_ID = "XLSX_IMPORT_ID";
    /**
     * The Constant ID.
     */
...
//    protected static final String ARRAY_ATTR_POSTFIX = SYSTEM_PATH_DELIMITER + "array";
    ...
    protected void addClassifiersToHeaders(List<String> classifierNames, List<XLSXHeader> headers) {
        ...
        for (String name : classifierNames) {
            ...
            headers.add(new XLSXHeader().withHrHeader(icp.getDisplayName()).withSystemHeader("CL_" + icp.getName() + "_node_name")
            ...
            List<ClsfNodeAttrDTO> clsfAttrs = classifierService.getAllClsfAttr(name);
            clsfAttrs.forEach(attr -> {
                final String typeName = findTypeName(attr);
                final XLSXHeader xlsxHeader = new XLSXHeader()
                        .withHrHeader(icp.getDisplayName() + H_R_PATH_DELIMITER + attr.getDisplayName())
                        .withSystemHeader("CL_" + icp.getName() + SYSTEM_PATH_DELIMITER + attr.getAttrName()/* + attrPostfix(attr)*/)
                        .withType(TYPE.CLASSIFIER_ATTRIBUTE);
                if (typeName != null) {
                    xlsxHeader.withTypeHeader(SimpleDataType.valueOf(typeName));
                }
                if (attr instanceof ClsfNodeArrayAttrDTO) {
                    xlsxHeader.withAttributeHolder(new AttributeInfoHolder(
                            JaxbUtils.getMetaObjectFactory().createArrayAttributeDef()
                                    .withArrayValueType(ArrayValueType.valueOf(typeName)),
                            null,
                            null,
                            null,
                            0
                ...
                else if (StringUtils.isNotBlank(((ClsfNodeSimpleAttrDTO) attr).getEnumDataType())) {
                    xlsxHeader.withAttributeHolder(new AttributeInfoHolder(
                            JaxbUtils.getMetaObjectFactory().createSimpleAttributeDef()
                                    .withEnumDataType(((ClsfNodeSimpleAttrDTO) attr).getEnumDataType()),
                            null,
                            null,
                            null,
                            0
                ...
                headers.add(xlsxHeader);
            });
        }
    }

    private String findTypeName(ClsfNodeAttrDTO attr) {
        if (attr.getDataType() != null) {
            return attr.getDataType().name();
        }
        if (attr.getLookupEntityCodeAttributeType() != null) {
            return attr.getLookupEntityCodeAttributeType().name();
        }
        if (attr instanceof ClsfNodeSimpleAttrDTO
                && StringUtils.isNotBlank(((ClsfNodeSimpleAttrDTO) attr).getEnumDataType())) {
            return SimpleDataType.STRING.name();
        }
        return null;
    }
