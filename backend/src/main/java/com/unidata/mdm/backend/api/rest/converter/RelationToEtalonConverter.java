            ...
            for(SearchResultHitDTO hit : searchResultDTO.getHits()){
                if(!processedRels.contains(hit.getId())){
                    ...
                    target.setCreateDate(SearchUtils.parse(Objects.toString(
                            hit.getFieldValue(RelationHeaderField.FIELD_CREATED_AT.getField()).getFirstValue(), null)));
                    target.setUpdateDate(SearchUtils.parse(Objects.toString(
                            hit.getFieldValue(RelationHeaderField.FIELD_UPDATED_AT.getField()).getFirstValue(), null)));

                    target.setValidFrom(ConvertUtils.date2LocalDateTime(SearchUtils.parse(Objects.toString(
                            hit.getFieldValue(RelationHeaderField.FIELD_FROM.getField()).getFirstValue(), null))));
                    target.setValidTo(ConvertUtils.date2LocalDateTime(SearchUtils.parse(Objects.toString(
                            hit.getFieldValue(RelationHeaderField.FIELD_TO.getField()).getFirstValue(), null))));
