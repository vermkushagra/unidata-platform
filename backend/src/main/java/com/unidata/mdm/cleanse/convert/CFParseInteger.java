 ...
 * @author ilya.bykov
 */
public class CFParseInteger extends BasicCleanseFunctionAbstract implements CFSystemCleanseFunction {
    ...
    public void execute(CleanseFunctionContext ctx) {
        ...
        if (valueParam.isEmpty()) {
            ...
            try {
                Long value = StringUtils.isBlank(valueString) ? null : Long.parseLong(valueString);
                ctx.putOutputParam(CleanseFunctionOutputParam.of(OUTPUT1, value));
            } catch (Exception e) {
                throw new CleanseFunctionExecutionException(ctx.getCleanseFunctionName(), e, "Unable to parse string to long");
            }
        }
