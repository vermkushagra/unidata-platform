...
public abstract class BasicCleanseFunctionAbstract implements CleanseFunction {
	 ...
	 *
	 */
	public abstract void execute(Map<String, Object> input, Map<String, Object> result)
			throws Exception;
	...
	protected void validateInput(Map<String, Object> input) throws CleanseFunctionExecutionException {
		...
		if (errorMessages.size() != 0) {
			throw new CleanseFunctionExecutionException(this.getDefinition().getFunctionName(), errorMessages);
		}
	}
			...
			if (StringUtils.equals(port.getName(), name)) {
				...
				if (port.isRequired() && (!input.containsKey(port.getName()) || value == null)) {
					throw new CleanseFunctionExecutionException(getDefinition().getFunctionName(),
							"Required value of " + port.getName() + " is missing!");
				}
				if (value == null) {
					return null;
