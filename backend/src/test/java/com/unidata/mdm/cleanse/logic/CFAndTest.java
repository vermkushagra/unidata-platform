...
public class CFAndTest {
	...
	public void testExecuteFALSE() throws Exception {
		Map<String, Object> input = new HashMap<>();
		BooleanSimpleAttributeImpl port1 = new BooleanSimpleAttributeImpl("port1", true);
		BooleanSimpleAttributeImpl port2 = new BooleanSimpleAttributeImpl("port2", false);
		input.put("port1", port1);
		input.put("port2", port2);
		Map<String, Object> output = cf.execute(input);
		assertNotNull(output);
	...
	public void testGetDefinition() {
		...
		assertEquals("and", cf.getDefinition().getFunctionName());
		assertEquals("com.unidata.mdm.cleanse.logic.CFAnd", cf.getDefinition().getJavaClass());
