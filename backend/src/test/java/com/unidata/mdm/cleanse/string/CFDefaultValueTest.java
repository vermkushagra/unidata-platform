 ...
 *
 */
public class CFDefaultValueTest {
	/** The cf. */
	...
	public void testExecuteTRUEInteger() throws Exception {
		...
		StringSimpleAttributeImpl port2 = new StringSimpleAttributeImpl("port2", "1234");
	...
	public void testExecuteTRUEString() throws Exception {
		...
		input.put("port1", port1);
		StringSimpleAttributeImpl port2 = new StringSimpleAttributeImpl("port2", "test");
		input.put("port2", port2);
		Map<String, Object> output = cf.execute(input);
		assertNotNull(output);
		assertEquals(1, output.size());
	...
	public void testGetDefinition() {
		...
		assertEquals("ЗначениеПоУмолчанию", cf.getDefinition().getFunctionName());
		assertEquals("com.unidata.mdm.cleanse.string.CFDefaultValue", cf.getDefinition().getJavaClass());
