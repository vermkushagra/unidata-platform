    ...
    convert: function (value) {
        ...
        if (record) {
            if (record.get(typeFieldName) === 'Number') {
                ...
                delim = Unidata.Config.getDecimalSeparator();
                serverDelim = Unidata.Config.getServerDecimalSeparator();

                if (value && delim !== serverDelim) {
                    value = Number(value.toString().replace(delim, serverDelim));
                }
