...
Ext.define('Unidata.view.steward.dataentity.attribute.array.ArrayAttributeRead', {
    ...
    formatValue: function (value) {
        ...
        if (dateTypes.indexOf(typeValue) !== -1) {
            ...
            switch (typeValue) {
                case 'Date':
                    value = Ext.Date.format(dateParsed, Unidata.Config.getDateFormat());
                    break;
                case 'Time':
                    value = Ext.Date.format(dateParsed, Unidata.Config.getTimeFormat());
                    break;
                case 'Timestamp':
                default:
                    value = Ext.Date.format(dateParsed, Unidata.Config.getDateTimeFormat());
            }
