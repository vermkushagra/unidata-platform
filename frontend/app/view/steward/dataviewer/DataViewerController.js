Ext.define('Unidata.view.steward.dataviewer.DataViewerController', {
    ...
    showDataViewer: function () {
        var view = this.getView(),
            dataCard = view.dataCard,
            customerCfg = Unidata.Config.getCustomerCfg();
    ...
    onSaveButtonClick: function () {
        var view = this.getView(),
            ...
            dataCard = view.dataCard,
            originCard = view.originCard,
            referencePanel = dataCard.referencePanel,
            timeIntervalContainer = dataCard.getTimeIntervalContainer(),
            needSaveDataRecord,
            needSaveReferences;
        ...
        Ext.defer(function () {
            needSaveDataRecord = dataRecord.checkDirty() || timeIntervalContainer.getIsCopyMode();
            needSaveReferences = referencePanel.isChanged();
            // перенос функционала из версии 3.8
            // если запись без изменений то необходимо сообщать об этом
            if (!needSaveDataRecord && !needSaveReferences) {
            ...
            if (needSaveDataRecord) {
                dataCard.saveDataRecord();
            } else if (needSaveReferences) {
                dataCard.saveReferenceRelations();
            }
