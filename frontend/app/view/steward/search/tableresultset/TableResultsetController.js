Ext.define('Unidata.view.steward.search.tableresultset.TableResultsetController', {
        ...
        this.toolTip = Ext.create('Ext.tip.ToolTip', {
            ...
            listeners: {
                beforeshow: function (tip) {
                    ...
                    if (view.destroying || view.isDestroyed) {
                        return false;
                    }
                    ...
                    if (column.column_id === '$period') {
                        ...
                        tipTemplate = Ext.create('Ext.Template' , [
                            '<b>' + Unidata.i18n.t('search>tableresultset.value') + ':</b> {value}',
                            '<br>',
                        ...
                        tipHtml = tipTemplate.apply({
                            value: value,
                            columnName: Unidata.i18n.t('glossary:timeintervals')
                        });
                    } else {
    ...
    restoreGridState: function (metaRecord, gridColumns) {
        ...
        // считаем что состояние не сохранено, если указано что компонент не должен запоминать свое состояние
        if (!view.stateful) {
            state = null;
        }
    ...
    updateSourceStore: function (source) {
        ...
        source.on('load', function (store) {
            view.fireEvent('storeload', store);
            viewModel.set('storeCount', store.getCount());
        }, this);
    },
    ...
    columnAutoWidth: function (column) {
        ...
        if (column.gapWidth) {
            gapWidth = column.gapWidth;
        }

        calcWidth = column.textEl.getWidth() + gapWidth;
