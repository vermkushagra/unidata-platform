...
Ext.define('Unidata.view.steward.search.bulk.wizard.settings.ModifyRecordsController', {
    ...
    onClassifiernodeschange: function (classifier, selectedClassifierNode) {
        ...
        if (selectedClassifierNode) {
            ...
            index = dataRecordClassifiers.findBy(function (classifierNode) {
                return classifierNode.get('classifierName') === classifierName &&
                    classifierNode.get('classifierNodeId') === null;
            });
    ...
    hideAttribute: function (record) {
        ...
        // комплексный атрибут со свойством hidden - не показываем
        if (Unidata.util.MetaAttribute.isComplexAttribute(record) && record.get('hidden')) {
            return true;
        }
