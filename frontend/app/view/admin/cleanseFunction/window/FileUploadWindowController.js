Ext.define('Unidata.view.admin.cleanseFunction.window.FileUploadWindowController', {
    ...
    onFileUpload: function (component) {
        var view = this.getView(),
            ...
            uploadGetParams,
            fileUploadField = view.fileUploadField,
            files,
            formData,
            btn, me,
            viewModel,
        ...
        uploadXhr.onload = function () {
            ...
            if (uploadXhr.status === 200 || uploadXhr.status === 202) {
                response = JSON.parse(uploadXhr.response);
    ...
    onFileUploadErrorChange: function (t, error) {
        var view = this.getView();

        view.errorContainer.setHidden(Ext.isEmpty(error));
    }
});
