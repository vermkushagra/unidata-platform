Ext.define('Unidata.view.admin.cleanseFunction.CleanseFunctionModel', {
    ...
    formulas: {
        ...
        supportedExecutionContextsStr: {
            bind: {
                supportedExecutionContexts: '{currentRecord.supportedExecutionContexts}'
            },
            get: function (getter) {
                var supportedExecutionContexts = getter.supportedExecutionContexts,
                    str = Unidata.i18n.t('admin.dq>execuionModeNotSet');

                if (Ext.isArray(supportedExecutionContexts) && supportedExecutionContexts.length > 0) {
                    str = Ext.Array.map(supportedExecutionContexts, function (supportedExecutionContext) {
                        return Unidata.util.DataQuality.executionContextLabels[supportedExecutionContext];
                    }).join(', ');
                }
