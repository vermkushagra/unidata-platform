...
Ext.define('Unidata.view.admin.entity.metarecord.attribute.AttributeController', {
    ...
    /**
     * Сгенерировать текст по умолчанию для редактора отображаемых атрибутов
     * @param metaRecord {Unidata.model.entity.Entity|Unidata.model.entity.LookupEntity}
     * @returns {String}
     */
    buildLookupEntityDefaultValueText: function (metaRecord) {
        ...
        displayNames = MetaAttributeFormatter.getMainDisplayableAttributeDisplayNames(metaRecord);
                ...
                lookupEntityDisplayAttributes: {
                    ...
                    renderer: this.buildRendererFunction(view.lookupEntityDisplayAttributesStore, null, null, {
                        fn: function () {
                            ...
                            defaultValueText = me.buildLookupEntityDefaultValueText(metaRecord);
    ...
    onAttributePropertyChange: function (source, recordId, value, oldValue) {
        ...
        //@TODO refactoring, NestedEntity set
        if (typeof record.getNestedEntity === 'function') {
            var nested = record.getNestedEntity();
        ...
        Ext.defer(this.focusPropertyGridRowByName, 50, this, [recordId, false]);
    },
    ...
    updateAttributeStore: function () {
        this.getViewModel().get('attributesStore').refreshDataFromMetaRecord();
    },
    ...
    updateMeasurementValuesStore: function () {
        ...
        promise = Unidata.util.api.Enumeration.loadStore(cacheMeasurementValues, true, storeLoadCfg);
        promise
            .then(
                function () {
                    var proxy;

                    cacheMeasurementValues = Unidata.util.api.MeasurementValues.getStore();
    ...
    updateTypeCategoriesStore: function () {
        var viewModel = this.getViewModel(),
            ...
            editor = propertyGrid.getConfigProp('simpleDataType', 'editor'),
            record,
            data,
            store;
    ...
    attributePhantom: function () {
        var attributeTreePanel = this.lookupReference('attributeTreePanel'),
            metaAttribute = attributeTreePanel.getSelection()[0].get('record');

        return metaAttribute.phantom;
    },

    attributeUsed: function () {
        if (this.attributePhantom()) {
            return false;
        }

        if (this.attributeUsedDq()) {
    ...
    onAttributeTreeNodeBeforeDeselect: function (tree, node) {
        ...
        editor = view.getLookupEntityDisplayAttributesEditor();
    ...
    onAttributeTreeNodeSelect: function () {
        var viewModel     = this.getViewModel(),
            attribute = this.getSelectedRecord(),
            view = this.getView(),
            generationStrategyForm = view.generationStrategyForm,
            attributeFilters;
    ...
    onAttributePropertyBeforeEdit: function (editor, context) {
        ...
        if (Ext.Array.contains(checkFieldName, propCodeName)) {
            ...
            if (me.attributeUsedAttributeGroup()) {
                canEdit = false;
            }
        }
    ...
    isAttributePropertyEditable: function (propCodeName) {
        ...
        attrRecord = attributeTreePanel.getSelection()[0].get('record');
        ...
        if (!attrRecord.phantom &&
            Ext.Array.contains(importantFields, propCodeName) &&
            (hasData || me.attributeUsedDq() || me.attributeUsedConsolidation() ||
            me.attributeUsedMatchingRules() || me.attributeUsedAttributeGroup() || me.attributeUsedLookupDisplayableAttributes())) {
            return false;
        }
        ...
        if (!attrRecord.phantom && hasData && propCodeName === 'defaultUnitId' &&
            !attrRecord.get('valueId') && attrRecord.get('simpleDataType') === 'Number') {
            return false;
        }

        if (isAliasCodeAttribute(attrRecord)) {
            if (Ext.Array.contains(aliasCodeAttributeFields, propCodeName)) {
                return false;
            }
        } else if (isCodeAttribute(attrRecord)) {
            if (Ext.Array.contains(codeAttributeFields, propCodeName)) {
                return false;
            }
        }

        if (isMainDisplayableAttribute(attrRecord)) {
            if (Ext.Array.contains(mainDisplayableFields, propCodeName)) {
                return false;
            }
        }

        if (isDisplayableAttribute(attrRecord)) {
            if (Ext.Array.contains(displayableFields, propCodeName)) {
                return false;
            }
        }

        if (isNotNullableAttribute(attrRecord)) {
            if (Ext.Array.contains(notNullableFields, propCodeName)) {
                return false;
            }
        }

        if (propCodeName === 'unique' && isNotUniqueAttribute(attrRecord) && hasData) {
            return false;
        }
    ...
    attributeUsedDq: function () {
        ...
        if (metaRecord.dataQualityRules) {
            metaRecord.dataQualityRules().each(function (dqRule) {
                if (!dqRule.get('special')) {
                    isAttributeUsed = isAttributeUsed || dqRule.isAttributeUsed(attributePath, true);
                    isAttributeUsed = isAttributeUsed || dqRule.isAttributeUsedByPath(attributePath, true);
                }
            });
        }
    ...
    processTabKey: function (event, currentPropertyName) {
        ...
        if (propertyName) {
            Ext.defer(this.focusPropertyGridRowByName, 50, this, [propertyName, true]);
        }
    },
