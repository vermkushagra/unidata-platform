...
Ext.define('Unidata.view.classifier.item.attribute.ClassifierAttributeController', {
    ...
    updateCustomPropertiesReadOnly: function (customPropertiesReadOnly) {
        var viewModel = this.getViewModel();

        viewModel.set('customPropertiesReadOnly', customPropertiesReadOnly);
    },
    ...
    getAttributeProperties: function () {
        ...
        // для строки и enum пустое значение должно быть интерпретировано как null
        if ((result['simpleDataType'] === 'String' || !Ext.isEmpty(result['enumDataType'])) && Ext.isEmpty(result['value'])) {
            result['value'] = null;
        }
    ...
    valueFieldFactoryEnumDataType: function () {
        var view = this.getView(),
            ...
            nodeAttribute = view.getNodeAttribute(),
            enumerationName = nodeAttribute.get('enumDataType'),
    ...
    highlightErrors: function () {
        ...
        if (Ext.isEmpty(view.typeCategoryField.getValue())) {
            ...
            switch (typeCategory) {
                ...
                case 'enumDataType': {
                    if (!nodeAttribute.isValidEnumTypeField()) {
                        view.enumDataTypeField.markInvalid(Unidata.i18n.t('classifier>notSelectEnum'));
                        hasError = true;
                    }
                    break;
                }
