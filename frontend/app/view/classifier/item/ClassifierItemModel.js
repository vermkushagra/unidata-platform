    ...
    formulas: {
            ...
            get: function (classifier) {
                ...
                if (classifier) {
                    ...
                    if (classifier.phantom && displayName === '') {
                        title = Unidata.i18n.t('classifier>newClassififer');
                    } else {
                        title = displayName;
                    }
                }
        ...
        classifierTreeHidden: {
            ...
            get: function (classifier) {
                var hidden,
                    classifierName,
                    //hasClassifierNodeReadRight = false,
                    phantom = true;

                if (classifier) {
                    //classifierName = classifier.get('name');
                    phantom = classifier.phantom;
                    //hasClassifierNodeReadRight = Unidata.Config.userHasRight(classifierName, 'read');
                }

                // права на чтение теперь не играют роли для классификаторов (см. баг UN-5604)
                //hidden = phantom || !hasClassifierNodeReadRight;
                hidden = phantom;
