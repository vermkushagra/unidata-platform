...
Ext.define('Unidata.view.component.dashboard.entity.items.ErrorsChart', {
    ...
    title: Unidata.i18n.t('dashboard>errorsStats'),
    ...
    items: [
        {
            ...
            height: 150,
            width: 150,
            html: '<div class="un-chart-total-count un-chart-total-count-large"></div><canvas class="un-chart-canvas"></canvas>'
        },
        {
            xtype: 'container',
            height: 150,
    ...
    updateChart: function () {
        var ErrorsChart = Unidata.view.component.dashboard.entity.items.ErrorsChart,
            chart = this.getChart(),
            dataset = chart.data.datasets[0],
            datasetData = dataset.data,
            errorsCount = 0,
            errorsCountDigits,
            cls = 'un-chart-total-count',
            totalCountTextEl;
        ...
        errorsCountDigits = String(errorsCount).length;
        totalCountTextEl = this.getEl().selectNode('.' + cls, false);
        totalCountTextEl.setText(errorsCount);
        totalCountTextEl.removeCls([ErrorsChart.SIZE.LARGE, ErrorsChart.SIZE.MEDIUM, ErrorsChart.SIZE.SMALL], cls);

        if (errorsCountDigits <= ErrorsChart.DIGITS.MILLION) {
            totalCountTextEl.addCls(cls + '-' + ErrorsChart.SIZE.LARGE);
        } else if (errorsCountDigits <= ErrorsChart.DIGITS.BILLION) {
            totalCountTextEl.addCls(cls + '-' + ErrorsChart.SIZE.MEDIUM);
        } else {
            totalCountTextEl.addCls(cls + '-' + ErrorsChart.SIZE.SMALL);
        }
