...
Ext.define('Unidata.view.component.dashboard.entity.items.EntityLineChartController', {
    ...
    onGranularityChange: function (combo, granularity) {
        this.loadStats();
    },
    ...
    updateChartAxes: function (maxY) {
        ...
        min = Ext.Date.format(
            view.startDateField.getValue(),
            'd.m.Y'
        );

        max = Ext.Date.format(
            view.endDateField.getValue(),
            'd.m.Y'
        );
        ...
        chart.options.scales.yAxes[0].ticks.max = (maxY + Math.ceil(maxY * 0.1)) || 10;
