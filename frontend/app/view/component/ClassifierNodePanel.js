...
Ext.define('Unidata.view.component.ClassifierNodePanel', {
    ...
    initListeners: function () {
        ...
        classifierTree.on('itemclick', this.onClassifierItemClick, this);
    },
    ...
    onClassifierItemClick: function (tree, node) {
    ...
    loadClassifierNodeById: function (classifierName, classifierNodeId) {
        var me = this,
            view = this.classifierTree,
            promise;
        ...
        promise = this.loadPromisedClassifier(classifierName)
            .then(
                function (classifier) {
                    view.setClassifier(classifier);

                    return view.displayClassifierNode(classifierNodeId);
                }
            ).then(
                function (classifierNodeMeta) {
                    return me.onClassifierItemClick(view, classifierNodeMeta);
                }
            );
    ...
    loadPromisedClassifier: function (classifierName) {
        var deferred = new Ext.Deferred(),
            classifierComboBox = this.lookupReference('classifierComboBox'),
            picker,
            store;
        ...
        if (store.isLoaded()) {
            ...
            deferred.resolve(picker.getSelection()[0]);
        } else {
    ...
    updateClassifierName: function (classifierName) {
        this.loadPromisedClassifier(classifierName);
    },
    ...
    selectClassifierByName: function (classifierName) {
        ...
        classifier = store.getDataSource().findBy(function (record) {
            return record.get('name') === classifierName;
        });
