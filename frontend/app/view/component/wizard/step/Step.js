...
Ext.define('Unidata.view.component.wizard.step.Step', {
    ...
    initItems: function () {
        ...
        if (!Ext.isEmpty(dockedButtons)) {
            this.getButtonToolbar().add(dockedButtons);
        }
    },
    ...
    /**
     * Функция вызываемая перед активацией следующего шага
     */
    beforeNextStepActivate: Ext.emptyFn,

    /**
     * Функция вызываемая перед активацией предыдущего шага
     */
    beforePrevStepActivate: Ext.emptyFn,
    ...
    /**
     * Возвращает тулбар содержащий кнопки
     *
     * @returns {*}
     */
    getButtonToolbar: function () {
        return this.lookupReference('buttonItems');
    }
