...
Ext.define('Unidata.view.component.button.RoundButton', {
    ...
    onComponentRender: function () {
        // если кнопка изначально скрыта, то после отображения она будет иметь не корректные размеры
        // смотри метод базового класса setSize
        if (!this.isVisible()) {
            this.on('show', this.initButtonSize, this, {single: true});
        } else {
            this.initButtonSize();
        }
