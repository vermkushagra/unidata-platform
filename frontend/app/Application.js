Ext.define('Unidata.Application', {
    ...
    onAuthenticate: function (authenticateData) {
        ...
        Unidata.Config.setLicense(authenticateData.license);
    ...
    onDeauthenticate: function () {
        ...
        Unidata.Config.setLicense(null);
    ...
    requireCustomerExternalClass: function () {
        var me = this,
            externalVendorLibUrls = Unidata.Config.getPlatformExternalVendorLibUrls(),
        ...
        urls = Ext.Array.merge(externalVendorLibUrls);

        // строим список адресов внешних классов каcтомера
        Ext.Array.each(externalClassNames, function (className) {
            var url;
        ...
        // на всякий случай выкидываем дубли
        urls = Ext.Array.unique(urls);
    ...
    checkCustomerConfig: function () {
        ...
        if (!Ext.isNumber(MERGE_RECORD_DISPLAY_COUNT)) {
            ...
            checkResults.push(
                Unidata.i18n.t('application>error.mergeRecordDisplayCountShouldInRange', {min: MIN_MERGE_RECORD_DISPLAY, max: MAX_MERGE_RECORD_DISPLAY})
            );
        }
    ...
    doAuthorization: function () {
        var urlParams = Ext.urlDecode(window.location.search.substring(1)), // window.location.search = '?params=123'
            sso = urlParams['sso'],
            token;

        token = Unidata.util.Router.getTokenValue('sso', 'token');

        if (!Ext.isEmpty(token)) {
            Unidata.Config.setToken(token);

            Unidata.util.Router.removeToken('sso');
        }
        ...
        if (sso === 'true' || sso === 'on') {
            this.doSsoAuthorization();
        // возможна авторизация по токену
        } else if (Unidata.Config.getToken() && Unidata.Config.getToken() !== 'null') {
            this.doTokenAuthorization();
        } else {
            Unidata.getApplication().showViewPort('login');
        }
    },
    ...
    doSsoAuthorization: function () {
        ...
        // sso авторизация производится с пустым логином / паролем т.к. эта авторизация кастомно реализуемая на BE
        // со своими критериями авторизации (например, авторизация по cookie)
    ...
    doTokenAuthorization: function () {
        ...
        promise = Unidata.util.api.Authenticate.authenticate();
