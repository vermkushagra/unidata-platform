...
Ext.define('Unidata.util.MetaRecord', {
    ...
    isMetaRecord: function (metaRecord) {
        return metaRecord instanceof Unidata.model.entity.AbstractEntity;
    },
     ...
     *
     * @param metaRecord
     * @param phantomOnly {Boolean} Если true, то возвращаем только новые комплексные атрибуты
     * @returns {*|{displayName, editor, renderer}|Array}
     */
    getAllComplexAttributes: function (metaRecord, phantomOnly) {
        var me         = this,
            attributes = [],
            complexAttributes;

        phantomOnly = Ext.isBoolean(phantomOnly) ? phantomOnly : false;
        ...
        complexAttributes.each(function (complexAttribute) {
            var metaNested            = complexAttribute.getNestedEntity(),
                nestedSimpleAttribute = me.getAllComplexAttributes(metaNested, phantomOnly);

            if (!phantomOnly || complexAttribute.phantom) {
