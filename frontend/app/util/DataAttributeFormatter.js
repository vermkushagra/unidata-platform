...
Ext.define('Unidata.util.DataAttributeFormatter', {
     ...
     * @param opt {Boolean} опции для выполнения операции
     *      opt = {
     *          applyDirtyPrefix: true | false - конкатенировать признак грязной записи или нет. true - конкатинировать
     *      }
     *
     * @returns {string} Заголовок
     */
    buildEntityTitleFromDataRecord: function (metaRecord, dataRecord, externalDirty, displayAttributes, useAttributeNameForDisplay, opt) {
        var title = '',
            values,
            delimiter = ' | ',
            dirty,
        ...
        if (dirty || externalDirty) {
            if (!Ext.isDefined(opt.applyDirtyPrefix) || opt.applyDirtyPrefix == true) {
    ...
    formatValueByAttribute: function (metaAttribute, value, parseFormats) {
        ...
        if (metaAttribute && value !== null && value !== undefined) {
            ...
            if (typeValue === 'Date' || typeValue === 'Timestamp' || typeValue === 'Time') {
                ...
                // на всякий случай, если придет в строковом виде
                if (Ext.isString(value)) {
                    if (value === 'true') {
                        value = true;
                    } else if (value === 'false') {
