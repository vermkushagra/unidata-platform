...
public class SoapJobApiServiceImpl implements SoapJobApiService {
    ...
    public void handleFindJob(final UnidataRequestBody request, final UnidataResponseBody response) {
        handleRequest(request, response, () -> {
            ...
            response.setResponseFindJob(
                    new ResponseFindJob().withJob(convertJobDTOToJob(jobService.findJob(jobId)))
            );
        });
    }
    ...
    @Override
    public void handleStopJob(final UnidataRequestBody request, final UnidataResponseBody response) {
        handleRequest(request, response, () -> {
            jobService.stopJob(request.getRequestStopJob().getId());
            response.setResponseStopJob(new ResponseStopJob());
        });
    }
