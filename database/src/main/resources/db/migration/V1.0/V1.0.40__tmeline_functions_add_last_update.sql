...
begin
    ...
    select case when a.date_point = '-infinity' then null else a.date_point end as valid_from, 
           ...
           ( select array_agg(row(t.origin_id, t.revision, (select source_system from origins where id = t.origin_id), t.status, t.approval, t.owner, to_char(t.last_update, 'YYYY-MM-DD HH24:MI:SS.MS'))::text) from t,
              ( select i.origin_id,             
                max(i.revision) as revision
                from t i
...
begin
    ...
    from ( 
        select case when a.date_point = '-infinity' then null else a.date_point end as valid_from, 
               case when b.date_point = 'infinity' then null else b.date_point end as valid_to,
               ( select array_agg(row(t.origin_id, t.revision, (select source_system from origins_relations where id = t.origin_id), t.status, t.approval, t.owner, to_char(t.last_update, 'YYYY-MM-DD HH24:MI:SS.MS'))::text) from t,
                  ( select i.origin_id,
                    max(i.revision) as revision
                    from t i
                    where coalesce(i.valid_from, '-infinity') <= a.date_point 
