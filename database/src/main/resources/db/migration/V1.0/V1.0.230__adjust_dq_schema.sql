...
CREATE OR REPLACE FUNCTION create_partition_and_insert() RETURNS trigger AS
  $BODY$
    DECLARE
      ...
      partition TEXT;
    BEGIN
      partition_n := abs(unidata_dq.h_int(NEW.record_id)%100)::int;
      partition := TG_RELNAME || '_' || partition_n;
      IF NOT EXISTS(SELECT relname FROM pg_class WHERE relname=partition) THEN
        RAISE NOTICE 'A partition has been created %',partition;
        EXECUTE 'CREATE TABLE unidata_dq.' || partition || ' (check (abs(unidata_dq.h_int(record_id)%100)::int = ' || partition_n || ')) INHERITS (unidata_dq.' || TG_RELNAME || ');';
