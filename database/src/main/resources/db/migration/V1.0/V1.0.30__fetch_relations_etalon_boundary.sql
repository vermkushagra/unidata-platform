...
drop function if exists fetch_relations_etalon_boundary(
    ...
    _relation_name character varying(256), 
...
begin
    ...
    select q.valid_from, q.valid_to, q.contributors, e.create_date, e.created_by,
        (select max(v.create_date) from origins_relations_vistory v, origins_relations o 
         where o.etalon_id = e.id 
         and v.origin_id = o.id) as update_date,
    ...
    from ( 
        select case when a.date_point = '-infinity' then null else a.date_point end as valid_from, 
               ...
               (select id from etalons_relations where etalon_id_from = _etalon_id and name = _relation_name) as etalon_id
