...
(
  ...
  category character varying(2044)[] COLLATE pg_catalog."POSIX",
...
$do$
BEGIN 
FOR partition_n IN 0..300 LOOP
    DECLARE
	TG_RELNAME TEXT := 'dq_results';
	partition TEXT:= 'dq_results' || '_' || partition_n;
	BEGIN
      EXECUTE 'CREATE TABLE unidata_dq.' || partition || ' (check (abs(unidata_dq.h_int(record_id)%300)::int = ' || partition_n || ')) INHERITS (unidata_dq.' || TG_RELNAME || ');';
      EXECUTE 'CREATE INDEX idx_dq_results_request_id' || partition_n || ' ON unidata_dq.' || partition || ' USING HASH  (request_id COLLATE pg_catalog."POSIX");';
      EXECUTE 'CREATE INDEX idx_dq_results_record_id' || partition_n || ' ON unidata_dq.' || partition || ' USING HASH  (record_id COLLATE pg_catalog."POSIX");';
      END;
END LOOP;
END
$do$;
...
CREATE OR REPLACE FUNCTION create_partition_and_insert() RETURNS trigger AS
  $BODY$
    DECLARE
      ...
      partition TEXT;
    BEGIN
      partition_n := abs(unidata_dq.h_int(NEW.record_id)%300)::int;
   ...
   --   END IF;
      EXECUTE 'INSERT INTO unidata_dq.' || partition || ' SELECT(unidata_dq.' || TG_RELNAME || ' ' || quote_literal(NEW) || ').* RETURNING record_id;';
      RETURN NULL;
    END;
  $BODY$
