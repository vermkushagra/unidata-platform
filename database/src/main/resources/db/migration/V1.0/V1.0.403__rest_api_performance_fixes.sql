...
-----------------------------------------------------------------------------------
-- drop function
drop function if exists ud_fetch_records_etalon_boundary(_etalon_id uuid, _ts timestamp with time zone, _user_name character varying(256), _is_approver boolean);
-----------------------------------------------------------------------------------
-- fn
...
begin
    ...
    --------------------- Join sorted
    select e.id, q.period_id, q.valid_from, q.valid_to, q.contributors, e.created_by,
        ...
        (select greatest(max(v.create_date), max(o.update_date), e.update_date) from origins_vistory v, origins o
        ...
        e.gsn as etalon_gsn
    from (
        select case when b.date_point = 'infinity' then 9223372036825200000 else (extract(epoch from b.date_point at time zone 'UTC') * 1000)::bigint end as period_id,
...
-- drop fn
drop function if exists ud_fetch_relations_etalon_boundary(_etalon_id uuid, _ts timestamp with time zone, _user_name character varying(256), _is_approver boolean);
-----------------------------------------------------------------------------------
-- create fn
create or replace function ud_fetch_relations_etalon_boundary(_etalon_id uuid, _ts timestamp with time zone, _user_name character varying(256), _is_approver boolean)
returns table(
    etalon_id uuid,
    period_id bigint,
    vf timestamp with time zone,
...
begin
    ...
    select e.id, q.period_id, q.valid_from, q.valid_to, q.contributors, e.created_by,
        ...
        (select greatest(max(v.create_date), max(o.update_date), e.update_date) from origins_relations_vistory v, origins_relations o
         where v.origin_id = o.id and o.etalon_id = e.id and (v.approval <> 'DECLINED' and (v.approval <> 'PENDING' or (_is_approver or v.created_by = _user_name)))
         ) as update_date,
        e.name,
        e.status,
...
$$ language plpgsql;
-----------------------------------------------------------------------------------
drop function if exists ud_fetch_records_timeline_intervals(_etalon_id uuid, _user_name character varying(256), _is_approver boolean);
-- fn
...
-----------------------------------------------------------------------------------
-----------------------------------------------------------------------------------
drop function if exists ud_fetch_relations_timeline_intervals(_etalon_id uuid, _user_name character varying(256), _is_approver boolean);
-----------------------------------------------------------------------------------
-- fn
