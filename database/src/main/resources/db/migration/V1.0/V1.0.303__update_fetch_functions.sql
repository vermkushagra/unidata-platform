...
begin
    ...
    select e.id, q.valid_from, q.valid_to, q.contributors, e.created_by,
        ...
        (select greatest(max(v.create_date), max(o.update_date)) from origins_vistory v, origins o
...
begin
    ...
    select e.id, q.valid_from, q.valid_to, q.contributors, e.created_by,
        ...
        (select greatest(max(v.create_date), max(o.update_date)) from origins_relations_vistory v, origins_relations o
         where v.origin_id = o.id and o.etalon_id = e.id and (v.approval <> 'DECLINED' and (v.approval <> 'PENDING' or (_is_approver or v.created_by = _user_name)))
         ) as update_date,
        e.name,
        e.status,
