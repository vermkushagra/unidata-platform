         ...
         <xsl:choose>
            ...
            <xsl:when test="normalize-space(translate($sortByComponent,'TRUE','true'))='true'">
               ...
               <xsl:if test="xsd:complexType">
                  <li><a href="#SchemaDefinitions">Верхнеуровневые элементы. Запрос-ответ</a>
                     <ul>
                        <xsl:apply-templates select="xsd:complexType[starts-with(@name,'Request') or starts-with(@name,'Response')]" mode="toc">
                           <xsl:sort select="local-name(.)" order="ascending"/>
                           <xsl:sort select="@name" order="ascending"/>
                        </xsl:apply-templates>
                     </ul>
                 ...
                 <!-- Other Definitions -->
                 <xsl:if test="xsd:attributeGroup or xsd:complexType or xsd:group or xsd:notation or xsd:simpleType">
                     <li><a href="#SchemaDefinitions">Остальные определения</a>
                         <ul>
                             <xsl:apply-templates select="xsd:attributeGroup | xsd:complexType[not(starts-with(@name,'Request')) and not(starts-with(@name,'Response'))] | xsd:group | xsd:notation | xsd:simpleType" mode="toc">
                                 <xsl:sort select="local-name(.)" order="ascending"/>
                                 <xsl:sort select="@name" order="ascending"/>
                             </xsl:apply-templates>
                         </ul>
