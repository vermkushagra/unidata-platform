package org.unidata.mdm.core.type.timeline.impl;
...
import org.unidata.mdm.core.type.timeline.impl.TimelineIterator.DateAdapter;
...
public class RevisionSlider<V extends Calculable> {
     ...
     */
    private static final Comparator<CalculableHolder<?>> POP_TOP_COMPARATOR =
    ...
    private boolean overlappedBy(CalculableHolder<V> me, CalculableHolder<V> by) {
        ...
        Date meFrom = Objects.isNull(me.getValidFrom()) ? PeriodIdUtils.TIMELINE_LOWER_BOUND : me.getValidFrom();
        Date byFrom = Objects.isNull(by.getValidFrom()) ? PeriodIdUtils.TIMELINE_LOWER_BOUND : by.getValidFrom();

        Date meTo = Objects.isNull(me.getValidTo()) ? PeriodIdUtils.TIMELINE_UPPER_BOUND : me.getValidTo();
        Date byTo = Objects.isNull(by.getValidTo()) ? PeriodIdUtils.TIMELINE_UPPER_BOUND : by.getValidTo();

        boolean fromOverlapped = byFrom.compareTo(meFrom) <= 0;
        boolean toOverlapped = byTo.compareTo(meTo) >= 0;

        return fromOverlapped && toOverlapped;
    }
}
