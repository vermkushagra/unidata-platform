...
import org.unidata.mdm.data.dao.rm.EtalonRecordDraftStateRowMapper;
import org.unidata.mdm.data.dao.rm.ExtendedRecordVistoryRowMapper;
import org.unidata.mdm.data.dao.rm.OriginRecordRowMapper;
import org.unidata.mdm.data.dao.rm.RecordKeysRowMapper;
import org.unidata.mdm.data.dao.rm.RecordTimelineRowMapper;
import org.unidata.mdm.data.dao.rm.RecordVistoryRowMapper;
 ...
 */
@Repository
public class DataRecordsDaoImpl extends BaseStorageDAOImpl implements DataRecordsDao {
    /**
     * Logger.
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(DataRecordsDaoImpl.class);
    /**
    ...
    enum DataRecordsQuery implements QueryTemplateDescriptor {
        ...
        DELETE_CDATA_BY_RECORD_ID_SQL("deleteCdataByRecordIdSQL", false),
        DELETE_BDATA_BY_RECORD_ID_SQL("deleteBdataByRecordIdSQL", false),
            ...
            if (isNew) {
                ...
                int[][] result = shardSelect(shard)
                        ...
                        (ps, record) -> {
                            ...
                            ps.setString(6, record.getApproval() == null ? ApprovalState.APPROVED.name()
                                    : record.getApproval().name());
                            ps.setString(7, record.getStatus() == null ? RecordStatus.ACTIVE.name()
                ...
                List<RecordEtalonPO> pendings = records.stream().filter(r -> r.getApproval() == ApprovalState.PENDING)
                ...
                if (!CollectionUtils.isEmpty(pendings)) {
                    ...
                    jdbcTemplate.batchUpdate(
                            ...
                            pendings, pendings.size(), (ps, pending) -> {
                        ps.setString(1, pending.getId());
                        ps.setString(2, pending.getId());
                        ps.setString(3,
                                pending.getStatus() == null ? RecordStatus.ACTIVE.name() : pending.getStatus().name());
                ...
                List<RecordEtalonPO> updates = records.stream().filter(r -> r.getApproval() != ApprovalState.PENDING)
                        .collect(Collectors.toList());

                for (RecordEtalonPO pending : pendings) {
                ...
                if (result[0].length != updates.size()) {
            ...
            upsert.stream()
                ...
                .forEach((shard, versions) -> {
                    ...
                    String statement = platformConfiguration.getDumpTargetFormat() == DumpTargetFormat.JAXB
                            ? templates.getQuery(DataRecordsQuery.PUT_VERSION_JAXB_SQL, shard)
                            : templates.getQuery(DataRecordsQuery.PUT_VERSION_PROTOSTUFF_SQL, shard);
                    ...
                    int[][] updates = shardSelect(shard)
                        ...
                        .batchUpdate(statement, versions, versions.size(), (ps, po) -> {
                            ...
                            if (platformConfiguration.getDumpTargetFormat() == DumpTargetFormat.JAXB) {
                                ps.setString(8, DataSerializer.dumpOriginRecordToJaxb(po.getData()));
                            ...
                            ps.setString(12, approval);
                            ps.setString(13, shift);
                            ps.setString(14, operationType);
                            ps.setString(15, po.getOperationId());
                            ps.setInt(16, platformConfiguration.getPlatformMajor());
                            ps.setInt(17, platformConfiguration.getPlatformMinor());
                        });
    ...
    public void bulkInsertEtalonRecords(int shard, List<RecordEtalonPO> records) {
        ...
        try {
            ...
            final String prolog = new StringBuilder()
                    ...
                    .append(shard)
                    .append(" (id, shard, name, create_date, created_by, status, approval) from stdin binary")
                    .toString();
            ...
            try (Connection connection = shardSelect(shard).dataSource().getConnection();
                ...
                for (RecordEtalonPO record : records) {
                    ...
                    params[6] = record.getApproval() == null ? ApprovalState.APPROVED.name()
                            : record.getApproval().name();
    ...
    public void bulkWipeRecordData(int shard, List<RecordKeysPO> records) {
        ...
        try (Connection c = shardSelect(shard).dataSource().getConnection()) {
            ...
            if (CollectionUtils.isNotEmpty(originIds)) {
                ...
                jdbcTemplate.update(templates.getQuery(DataRecordsQuery.DELETE_CDATA_BY_RECORD_ID_SQL), oia);
                jdbcTemplate.update(templates.getQuery(DataRecordsQuery.DELETE_BDATA_BY_RECORD_ID_SQL), oia);
    ...
    public boolean changeEtalonApproval(String etalonId, ApprovalState approval) {
        ...
        try {
            ...
            int shard = StorageUtils.shard(UUID.fromString(etalonId));
            int rowCount = shardSelect(shard)
                .jdbcTemplate()
                .update(templates.getQuery(DataRecordsQuery.CHANGE_ETALON_APPROVAL_SQL, shard),
                    SecurityUtils.getCurrentUserName(),
                    approval != null ? approval.name() : ApprovalState.APPROVED.name(), etalonId,
    ...
    */
    ...
    public List<RecordVistoryPO> loadVersionsUnfilterdByEtalonId(String id, Date point) {
        ...
        final RowMapper<RecordVistoryPO> rm;
        if (platformConfiguration.getDumpTargetFormat() == DumpTargetFormat.JAXB) {
            rm = RecordVistoryRowMapper.DEFAULT_JAXB_ROW_MAPPER;
        } else {
            rm = RecordVistoryRowMapper.DEFAULT_PROTOSTUFF_ROW_MAPPER;
        }
    ...
    public Map<RecordOriginKeyPO, List<RecordVistoryPO>> loadHistory(String etalonId) {
        ...
        final RowMapper<RecordVistoryPO> rm;
        if (platformConfiguration.getDumpTargetFormat() == DumpTargetFormat.JAXB) {
            rm = ExtendedRecordVistoryRowMapper.DEFAULT_EXTENDED_RECORD_VISTORY_JAXB_ROW_MAPPER;
        } else {
            rm = ExtendedRecordVistoryRowMapper.DEFAULT_EXTENDED_RECORD_VISTORY_PROTOSTUFF_ROW_MAPPER;
        }
     ...
     */
    @Override
    public RecordTimelinePO loadTimeline(UUID val, boolean fetchKeys, boolean fetchData, boolean includeDrafts) {
     ...
     */
    @Override
    public RecordTimelinePO loadTimeline(ExternalId externalId, boolean fetchKeys, boolean fetchData, boolean includeDrafts) {
        ...
        try {
            ...
            return loadTimeline(sysId, fetchKeys, fetchData, includeDrafts);
        } finally {
            MeasurementPoint.stop();
        }
    }
     ...
     */
    @Override
    public RecordTimelinePO loadTimeline(LSN lsnId, boolean fetchKeys, boolean fetchData, boolean includeDrafts) {
    ...
    public Map<String, RecordTimelinePO>
        loadTimelines(List<String> etalonIds, boolean fetchKeys, boolean fetchData, boolean includeDrafts) {
        ...
        try {
            ...
            return etalonIds.stream()
                .filter(Objects::nonNull)
                .map(id -> Pair.of(id, loadTimeline(UUID.fromString(id), fetchKeys, fetchData, includeDrafts)))
    ...
    public boolean updateApprovalState(String recordEtalonId, ApprovalState to) {
        ...
        try {
            ...
            UUID val = UUID.fromString(recordEtalonId);
            int shard = StorageUtils.shard(val);
            shardSelect(shard)
                .jdbcTemplate()
                .update(templates.getQuery(DataRecordsQuery.UPDATE_PENDING_VERSIONS_SQL, shard),
                        to == null ? ApprovalState.APPROVED.name() : to.name(), recordEtalonId);
            return true;
        } finally {
            MeasurementPoint.stop();
        }
