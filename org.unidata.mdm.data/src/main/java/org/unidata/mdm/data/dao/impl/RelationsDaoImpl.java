...
import org.unidata.mdm.core.dao.template.QueryTemplateDescriptor;
import org.unidata.mdm.core.dao.template.QueryTemplates;
...
import org.unidata.mdm.data.dao.rm.EtalonRelationDraftStateRowMapper;
import org.unidata.mdm.data.dao.rm.EtalonRelationRowMapper;
import org.unidata.mdm.data.dao.rm.RelationKeysRowMapper;
import org.unidata.mdm.data.dao.rm.RelationTimelineRowMapper;
import org.unidata.mdm.data.dao.rm.RelationVistoryRowMapper;
...
public class RelationsDaoImpl extends BaseStorageDAOImpl implements RelationsDao {
    ...
    enum RelationRecordsQuery implements QueryTemplateDescriptor {
        ...
        UPDATE_PENDING_VERSIONS_SQL("updatePendingVersionsSQL", true),
        ...
        CHANGE_ETALON_APPROVAL_SQL("changeEtalonApprovalSQL", true),
    ...
    public RelationsDaoImpl(
        ...
        registerInitHook(this::initHook);
    }
    ...
    private void postProcessContainmentVersions(RelationTimelinePO timeline, Date asOf, String operationId, boolean includeDraftVersions) {
        ...
        for (RecordVistoryPO rvpo : rtl.getVistory()) {
            ...
            converted.setApproval(rvpo.getApproval());
     ...
     */
    // TODO: Remove this after FPD implemented.
    private void postProcessContainmentTimeline(RelationTimelinePO timeline, boolean fetchData, boolean includeDraftVersions) {
        ...
        RecordTimelinePO rtl = dataRecordsDAO.loadTimeline(
                UUID.fromString(keys.getToKeys().getId()), false, fetchData, includeDraftVersions);
     ...
     */
    @Override
    public List<RelationKeysPO> loadAllKeysByRecordEtalonId(UUID recordEtalonId, RelationSide side) {
        // TODO Implement!!!
     ...
     */
    @Override
    public RelationTimelinePO loadTimeline(UUID etalonId, boolean fetchKeys, boolean fetchData, boolean includeDrafts) {

        MeasurementPoint.start();
        try {

            // etalon_id, approver, user_name, approver, user_name, fetch_keys, fetch_data, fetch_data, shard, etalon_id
            ...
            RelationTimelinePO result = shardSelect(shard)
                    ...
                    .query(templates.getQuery(RelationRecordsQuery.LOAD_TIMELINE_BY_ETALON_ID_SQL, shard),
                            ...
                            etalonId, includeDrafts, user, includeDrafts, user,
                            fetchKeys,
                            fetchData,
                            fetchData,
                            shard,
            ...
            if (Objects.nonNull(result) && Objects.nonNull(result.getKeys()) && Objects.nonNull(result.getKeys().getToKeys())) {
                ...
                postProcessContainmentTimeline(result, fetchData, includeDrafts);
            }
     ...
     */
    @Override
    public RelationTimelinePO loadTimeline(LSN lsn, boolean fetchKeys, boolean fetchData, boolean includeDrafts) {
        MeasurementPoint.start();
        try {

            // shard, lsn, approver, user_name, approver, user_name, fetch_keys, fetch_data, fetch_data, shard, shard, lsn
            String user = SecurityUtils.getCurrentUserName();
            RelationTimelinePO result = shardSelect(lsn.getShard())
                    ...
                    .query(templates.getQuery(RelationRecordsQuery.LOAD_TIMELINE_BY_LSN_SQL, lsn.getShard()),
                            rs -> rs.next() ? RelationTimelineRowMapper.DEFAULT_RELATION_TIMELINE_ROW_MAPPER.mapRow(rs, 0) : null,
                            lsn.getShard(), lsn.getLsn(), includeDrafts, user, includeDrafts, user,
                            fetchKeys,
                            fetchData,
                            fetchData,
                            lsn.getShard(),
     ...
     */
    @Override
    public RelationTimelinePO loadTimeline(LSN from, LSN to, String relationName, boolean fetchData, boolean includeDrafts) {
        MeasurementPoint.start();
        try {
            ...
            return loadTimeline(target, true, fetchData, includeDrafts);
        } finally {
            MeasurementPoint.stop();
        }
    }
     ...
     */
    @Override
    public RelationTimelinePO loadTimeline(ExternalId from, ExternalId to, String relationName, boolean fetchData, boolean includeDrafts) {
        MeasurementPoint.start();
        try {
     ...
     */
    @Override
    public RelationTimelinePO loadTimeline(UUID from, UUID to, String relationName, boolean fetchData, boolean includeDrafts) {
        MeasurementPoint.start();
        try {
    ...
    public List<RelationTimelinePO>
        loadTimelines(UUID recordEtalonId, List<String> names, boolean isToSide, boolean fetchData, boolean includeDrafts) {
     ...
     */
    @Override
    public List<RelationEtalonPO> loadEtalonRelations(UUID recordEtalonId, String relationName, List<RecordStatus> statuses, RelationSide side) {
        MeasurementPoint.start();
        try {
    ...
    public RelationDigestDTO loadDigestDestinationEtalonIds(UUID recordEtalonId, String relationName,
        ...
        if (CollectionUtils.isNotEmpty(relations)) {
            ...
            result = relations.stream()
                    .map(relation -> viewSide == RelationSide.TO ? relation.getToEtalonId() : relation.getFromEtalonId())
                    .collect(Collectors.toList());
        }
                ...
                for (Entry<Boolean, List<RelationEtalonPO>> entry : split.entrySet()) {
                    ...
                    shardSelect(shard)
                        ...
                        .batchUpdate(templates.getQuery(RelationRecordsQuery.INSERT_ETALON_RELATION_SQL, shard), etalons, etalons.size(), (ps, etalon) -> {
                            ...
                            ps.setString(9, etalon.getApproval() == null ? ApprovalState.APPROVED.name() : etalon.getApproval().name());
                            ps.setString(10, etalon.getRelationType() == null ? null : etalon.getRelationType().name());
                            ps.setString(11, etalon.getOperationId());
                        });
            ...
            // Update
                ...
                for (Entry<Boolean, List<RelationEtalonPO>> entry : split.entrySet()) {
                    ...
                    if (entry.getKey()) {
                        ...
                        changeEtalonApproval(entry.getValue().stream()
                                .map(RelationEtalonPO::getId)
                                .map(UUID::fromString)
                                .collect(Collectors.toList()), ApprovalState.PENDING);
                        ...
                        shardSelect(shard)
                            ...
                            .batchUpdate(templates.getQuery(RelationRecordsQuery.UPDATE_ETALON_RELATION_SQL, shard), etalons, etalons.size(), (ps, etalon) -> {
                                ...
                                ps.setInt(6, etalon.getShard());
                                ps.setObject(7, UUID.fromString(etalon.getId()));
                            });
                    }
                }
        ...
        try {
            ...
            upsert.stream()
                ...
                String statement = platformConfiguration.getDumpTargetFormat() == DumpTargetFormat.JAXB
                        ? templates.getQuery(RelationRecordsQuery.PUT_RELATION_VERSION_JAXB_SQL, shard)
                        : templates.getQuery(RelationRecordsQuery.PUT_RELATION_VERSION_PROTOSTUFF_SQL, shard);
                ...
                int[][] result = shardSelect(shard)
                    ...
                    .batchUpdate(statement, versions, versions.size(), (ps, version) -> {
                        ...
                        ps.setString(14, version.getOperationId());
                        ps.setInt(15, version.getMajor());
                        ps.setInt(16, version.getMinor());
                    });
     ...
     */
    @Override
    public boolean changeEtalonApproval(List<UUID> relationEtalonIds, ApprovalState approval) {

        MeasurementPoint.start();
        try {

            String userName = SecurityUtils.getCurrentUserName();
            relationEtalonIds.stream()
                .collect(Collectors.groupingBy(StorageUtils::shard))
                .entrySet()
                .forEach(entry -> {

                    try (Connection cn = shardSelect(entry.getKey()).dataSource().getConnection()) {

                        Array ids = cn.createArrayOf("uuid", entry.getValue().toArray());
                        shardSelect(entry.getKey())
                            .jdbcTemplate()
                            .update(templates.getQuery(RelationRecordsQuery.CHANGE_ETALON_APPROVAL_SQL, entry.getKey()),
                                    userName,
                                    approval != null ? approval.name() : ApprovalState.APPROVED.name(),
                                    ids);

                    } catch (SQLException sqle) {
                        LOGGER.warn("Cannot create UUID array.", sqle);
                    }
                });
    ...
    public void bulkInsertEtalonRecords(int shard, List<RelationEtalonPO> records) {
        ...
        try {
            ...
            final String prolog = new StringBuilder().append("copy relation_etalons_p")
                    .append(shard)
                    .append(" (shard, id, name, etalon_id_from, etalon_id_to, create_date, created_by, status, approval, reltype, operation_id) from stdin binary")
                    .toString();
            ...
            try (Connection connection = shardSelect(shard).dataSource().getConnection();
                ...
                for (RelationEtalonPO record : records) {
                    ...
                    params[8] = record.getApproval() == null ? ApprovalState.APPROVED.name() : record.getApproval().name();
                    params[9] = record.getRelationType() == null ? null : record.getRelationType().name();
    ...
    public void bulkInsertVersions(int shard, List<RelationVistoryPO> versions) {
        ...
        try {
            ...
            final String statement = new StringBuilder()
                    .append("copy relation_vistory_p")
                    .append(shard)
                    .append(" (id, origin_id, shard, revision, valid_from, valid_to,")
                    .append(platformConfiguration.getDumpTargetFormat() == DumpTargetFormat.JAXB ? " data_a," : " data_b,")
                    .append(" create_date, created_by, status, approval, shift, operation_type, operation_id, major, minor) from stdin binary")
                    .toString();
            ...
            try (Connection connection = shardSelect(shard).dataSource().getConnection();
                ...
                for (RelationVistoryPO record : versions) {
                    ...
                    if (platformConfiguration.getDumpTargetFormat() == DumpTargetFormat.JAXB) {
                        params[6] = DataSerializer.dumpOriginRecordToJaxb(record.getData());
                    } else if (platformConfiguration.getDumpTargetFormat() == DumpTargetFormat.PROTOSTUFF) {
                    ...
                    params[10] = record.getApproval() == null ? ApprovalState.APPROVED.name() : record.getApproval().name();
                    params[11] = record.getShift() == null ? DataShift.PRISTINE.name() : record.getShift().name();
                    params[12] = record.getOperationType() == null ? OperationType.DIRECT.name() : record.getOperationType().name();
                    params[13] = record.getOperationId();
                    params[14] = record.getMajor();
                    params[15] = record.getMinor();
        ...
        try {
            ...
            try (Connection connection = shardSelect(shard).dataSource().getConnection();
                ...
                for (RelationVistoryPO record : versions) {
                    ...
                    if (platformConfiguration.getDumpTargetFormat() == DumpTargetFormat.JAXB) {
                        params[6] = record.getJaxbRawData();
                    } else if (platformConfiguration.getDumpTargetFormat() == DumpTargetFormat.PROTOSTUFF) {
                    ...
                    params[10] = record.getApproval().name();
                    params[11] = record.getShift().name();
                    params[12] = record.getOperationType().name();
                    params[13] = record.getOperationId();
                    params[14] = record.getMajor();
                    params[15] = record.getMinor();
     ...
     */
    @Override
    public boolean updateApprovalState(UUID relationEtalonId, ApprovalState to) {
        MeasurementPoint.start();
        try {
            ...
            shardSelect(shard)
                .jdbcTemplate()
                .update(templates.getQuery(RelationRecordsQuery.UPDATE_PENDING_VERSIONS_SQL, shard),
